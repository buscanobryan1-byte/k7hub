...
local function startMedusaCounter()
    ...
    table.insert(medusaCounterConns,c.DescendantAdded:Connect(function(obj)
        ...
        pcall(function() if isMedusaObj(obj.Name) then obj:Destroy() end end)
    end))
    ...
    table.insert(medusaCounterConns,RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            for _,obj in ipairs(ch:GetDescendants()) do
                if isMedusaObj(obj.Name) then pcall(function() obj:Destroy() end) end
            end
...
do
    local speedOpen=true
    local sHdr=Instance.new("TextButton",FeatFrame)
    sHdr.Name="SpeedHdr" sHdr.Size=UDim2.new(1,0,0,40) sHdr.BackgroundColor3=Color3.fromRGB(18,14,32)
    sHdr.BorderSizePixel=0 sHdr.LayoutOrder=9 sHdr.Text="" sHdr.AutoButtonColor=false
    Instance.new("UICorner",sHdr).CornerRadius=UDim.new(0,10)
    local sStroke=Instance.new("UIStroke",sHdr) sStroke.Color=ACCENT sStroke.Thickness=1
    local sTitleLbl=Instance.new("TextLabel",sHdr)
    sTitleLbl.Size=UDim2.new(1,-42,1,0) sTitleLbl.Position=UDim2.new(0,14,0,0) sTitleLbl.BackgroundTransparency=1
    sTitleLbl.Text="⚡  Speed Customizer" sTitleLbl.TextColor3=WHITE sTitleLbl.Font=Enum.Font.GothamBold
    sTitleLbl.TextSize=13 sTitleLbl.TextXAlignment=Enum.TextXAlignment.Left
    local sArrow=Instance.new("TextLabel",sHdr)
    sArrow.Size=UDim2.new(0,28,1,0) sArrow.Position=UDim2.new(1,-34,0,0) sArrow.BackgroundTransparency=1
    sArrow.Text="▲" sArrow.TextColor3=ACCENT sArrow.Font=Enum.Font.GothamBold sArrow.TextSize=13
    local sBody=Instance.new("Frame",FeatFrame)
    sBody.Size=UDim2.new(1,0,0,102) sBody.BackgroundColor3=Color3.fromRGB(11,11,18)
    sBody.BorderSizePixel=0 sBody.LayoutOrder=10 sBody.Visible=true
    Instance.new("UICorner",sBody).CornerRadius=UDim.new(0,10)
    local sBodyLayout=Instance.new("UIListLayout",sBody)
    sBodyLayout.Padding=UDim.new(0,4) sBodyLayout.HorizontalAlignment=Enum.HorizontalAlignment.Center
    local sPad=Instance.new("UIPadding",sBody)
    sPad.PaddingTop=UDim.new(0,7) sPad.PaddingBottom=UDim.new(0,7) sPad.PaddingLeft=UDim.new(0,10) sPad.PaddingRight=UDim.new(0,10)
    local function makeSpeedRow(labelText,getValue,setValue)
        local row=Instance.new("Frame",sBody)
        row.Size=UDim2.new(1,0,0,38) row.BackgroundColor3=CARD row.BorderSizePixel=0
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
        local lbl=Instance.new("TextLabel",row)
        lbl.Size=UDim2.new(0.52,0,1,0) lbl.Position=UDim2.new(0,12,0,0) lbl.BackgroundTransparency=1
        ...
        local box=Instance.new("TextBox",row)
        box.Size=UDim2.new(0.4,0,0.65,0) box.Position=UDim2.new(0.57,0,0.175,0)
        box.BackgroundColor3=Color3.fromRGB(20,18,32) box.Text=tostring(getValue())
        box.TextColor3=ACCENT box.Font=Enum.Font.GothamBold box.TextSize=15
        box.TextXAlignment=Enum.TextXAlignment.Center box.BorderSizePixel=0 box.ClearTextOnFocus=false
    ...
    makeSpeedRow("Speed",function() return NORMAL_SPEED end,function(n) NORMAL_SPEED=n end)
    makeSpeedRow("Carry",function() return SLOW_SPEED end,function(n) SLOW_SPEED=n end)
    sHdr.MouseButton1Click:Connect(function()
        speedOpen=not speedOpen sBody.Visible=speedOpen
        sArrow.Text=speedOpen and "▲" or "▼"
    end)
end
...
makeNumInput(SetFrame,"Carry Speed",SLOW_SPEED,3,function(v) SLOW_SPEED=v end)
