...
local function loadConfig()
    pcall(function()
        ...
        local fields = {
            ...
            "fovValue","AIMBOT_SPEED","BAT_ENGAGE_RANGE"
        }
...
local function makeSpeedBB()
    ...
    speedBB.Name = "EclipseSpeedBB"; speedBB.Adornee = head
...
local function createESP(plr)
    ...
    box.Name="EclipseESP"; box.Adornee=root; box.Size=v3new(4,6,2)
    box.Color3=ACCENT; box.Transparency=0.45; box.ZIndex=10; box.AlwaysOnTop=true; box.Parent=c

    local bb  = Instance.new("BillboardGui")
    bb.Name="EclipseESP_Name"; bb.Adornee=head
...
local function removeESP(plr)
    ...
    local b = plr.Character:FindFirstChild("EclipseESP")
    local n = plr.Character:FindFirstChild("EclipseESP_Name")
...
local function AP_StartSide(
    ...
    setConn(RunService.Heartbeat:Connect(function()
        ...
        local mv = d.Unit
        hum:Move(mv, false)
        rp.AssemblyLinearVelocity = v3new(mv.X*spd, rp.AssemblyLinearVelocity.Y, mv.Z*spd)
    end))
end
...
local function isHoldingBrainrot()
    ...
    local IGNORE = {
        ...
        RightUpperLeg=true,Animate=true,EclipseSpeedBB=true,
    }
...
TitleSep.Size=udim2new(0,12,1,-2); TitleSep.Position=udim2new(0,120,0,0); TitleSep.BackgroundTransparency=1
TitleSep.Text="|"; TitleSep.Font=Enum.Font.Gotham; TitleSep.TextSize=13; TitleSep.TextColor3=ACCENT
...
TitleStats.Size=udim2new(1,-140,1,-2); TitleStats.Position=udim2new(0,134,0,0); TitleStats.BackgroundTransparency=1
TitleStats.Text="FPS --  PING --ms"; TitleStats.Font=Enum.Font.GothamBold; TitleStats.TextSize=11; TitleStats.TextColor3=PINK_TEXT
TitleStats.TextXAlignment=Enum.TextXAlignment.Left; TitleStats.TextYAlignment=Enum.TextYAlignment.Center

task.spawn(function()
    local lastT=tickFn(); local frames=0
    RunService.RenderStepped:Connect(function()
        frames = frames+1
        local now = tickFn()
        if now-lastT >= 0.5 then
            local fps = mround(frames/(now-lastT)); frames=0; lastT=now
            local ping = 0
            pcall(function() ping=mround(Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) end)
...
do
    ...
    sfDot.Size=udim2new(0,dSz,0,dSz); sfDot.Position=udim2new(1,-dSz-3,0.5,-dSz/2)
...
do
    ...
    PBC.Size     = udim2new(0,220,0,38)
    PBC.Position = SavedPBCPos
        and udim2new(SavedPBCPos[1],SavedPBCPos[2],SavedPBCPos[3],SavedPBCPos[4])
        or  udim2new(0.5,-110,0,8)
    StealBarRef  = PBC
    ...
    hubLbl.Size=udim2new(1,-20,0,20); hubLbl.Position=udim2new(0,10,0,4)
    ...
    ProgressLabel.Size=udim2new(0,0,0,0); ProgressLabel.BackgroundTransparency=1; ProgressLabel.Text="READY"
    ...
    ProgressPctLabel.Size=udim2new(0,0,0,0); ProgressPctLabel.BackgroundTransparency=1; ProgressPctLabel.Text=""
    ProgressPctLabel.Visible=false; ProgressPctLabel.ZIndex=3
    RadiusInput=Instance.new("TextBox",PBC)
    RadiusInput.Size=udim2new(0,28,0,12); RadiusInput.Position=udim2new(1,-32,1,-14)
    ...
    RadiusInput.FocusLost:Connect(function()
        ...
        if n then
            STEAL_RADIUS=mclamp(mfloor(n),5,200)
    ...
    pt.Size=udim2new(1,-20,0,4); pt.Position=udim2new(0,10,1,-6)
    ...
    ProgressBarFill.Size=udim2new(0,0,1,0); ProgressBarFill.BackgroundColor3=ACCENT
        ...
        local conn; conn=input.Changed:Connect(function()
            if input.UserInputState==Enum.UserInputState.Change then
                if pbDragging then
                    ...
                    PBC.Position=udim2new(pbStartPos.X.Scale,pbStartPos.X.Offset+d.X,pbStartPos.Y.Scale,pbStartPos.Y.Offset+d.Y)
                end
...
do
    ...
    local function makeMob(label,initPos,toggleName,onAct)
        ...
        btn.Size=udim2new(0,58,0,58)
        local posKey=label:gsub("\n","_")
        local saved=mobBtnPositions[posKey]
        btn.Position=saved and udim2new(saved[1],saved[2],saved[3],saved[4]) or initPos
            ...
            local conn; conn=input.Changed:Connect(function()
                if input.UserInputState==Enum.UserInputState.Change then
                    ...
                    if not mobBtnsLocked and d.Magnitude>DRAG_THRESHOLD then
                        moved=true
                        btn.Position=udim2new(startPos.X.Scale,startPos.X.Offset+d.X,startPos.Y.Scale,startPos.Y.Offset+d.Y)
                    end
                elseif input.UserInputState==Enum.UserInputState.End then
                    conn:Disconnect()
                    if moved then
    ...
    local col1 = udim2new(1,-120,0.5,-170); local gap = 68
    local dropMobBtn=makeMob("DROP",col1,nil,function() task.spawn(doDrop) end)
    dropMobBtn.Visible=true; dropMobBtnRef=dropMobBtn; dropMobBtn.Size=udim2new(0,90,0,50)

    local autoBatBtn=makeMob("LOCK",udim2new(1,-120,0.5,-170+gap),"Lock",function()
    ...
    autoBatBtn.Size=udim2new(0,90,0,50)
    ...
    do
        local apBtn=makeMob("AUTO\nPLAY",udim2new(1,-120,0.5,-170),nil,function() AP_Toggle() end)
        apBtn.Size=udim2new(0,90,0,50)
        local apSaved=mobBtnPositions["AutoPlay_Btn"]
        if apSaved then apBtn.Position=udim2new(apSaved[1],apSaved[2],apSaved[3],apSaved[4]) end
    ...
    local tpMobBtn=makeMob("TP\nDOWN",udim2new(1,-120,0.5,-170+gap*2),nil,function() task.spawn(doTPDown) end)
    tpMobBtnRef=tpMobBtn; tpMobBtn.Size=udim2new(0,90,0,50)
end
