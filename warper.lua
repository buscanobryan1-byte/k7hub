...
local function applyXray(plot)
    ...
    for _, part in ipairs(plot:GetDescendants()) do
        if part:IsA("BasePart") then
            ...
            -- Si tenÃ­a highlight viejo del Xray, eliminarlo
    ...
    -- Detectar regeneraciÃ³n interna
...
function enableAntiFPSDevourer()
    ...
    -- ð¥ Escanear TODO el workspace (jugadores, clones, NPCs)
    ...
    -- ð¥ Detectar nuevos objetos (Quantum Cloner incluido)
...
-- ======================
-- AUTO WALK con GUI de rutas (versiÃ³n corregida)
-- ======================
...
local function createAutoWalkGui()
    ...
    -- Limpieza automÃ¡tica al respawnear
...
local combatFuncs = {"Melee Aimbot","Auto Steal Nearest","Auto Walk","Lock Target","Auto Medusa","Auto Bat","Anti Sentry","Auto Play"}
...
do
    ...
    local function showBoxes()
        ...
        for i, wp in ipairs(pts) do
            ...
            part.BrickColor = BrickColor.new(boxColor == Color3.fromRGB(255, 200, 0) and "Bright blue" or "Bright red")
    ...
    -- Close button also clears boxes
    closeBtn.MouseButton1Click:Connect(function()
        panel.Visible = false
        clearBoxes()
    end)
...
local function createAutoPlayUI()
    -- AutoPlay draggable button (parented directly to main hub ScreenGui)
...
-- Optimized: walk the known Plots folder directly instead of GetDescendants()
local function getPlotOwner(plotPart)
    if not plotPart then return "Unknown" end
    local sg2 = plotPart:FindFirstChildOfClass("SurfaceGui")
        or plotPart:FindFirstChild("PlotSign") and plotPart.PlotSign:FindFirstChildOfClass("SurfaceGui")
    if sg2 then
        for _, lbl in ipairs(sg2:GetDescendants()) do
...
local function checkMyPlot()
    ...
    for plotNum, position in pairs(AutoPlay.plotPositions) do
        for _, plot in ipairs(plots:GetChildren()) do
            ...
            if sign and (sign.Position - position).Magnitude < 1 then
                local owner = getPlotOwner(sign)
                if string.find(owner, lp.Name, 1, true) or string.find(owner, lp.DisplayName, 1, true) then
                    return plotNum
                end
            end
        end
...
local function enableAutoPlay()
    ...
    AutoPlay.btn.Text = "AutoPlay: OFF"
    -- Cache character references to avoid per-frame FindFirstChild calls
    local _cachedChar, _cachedHRP, _lastCharCheck = nil, nil, 0
    local _lastStep = 0
    local STEP_RATE = 1/20 -- 20hz is plenty for movement, avoids render-thread spikes

    AutoPlay.renderConn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Refresh character cache every 0.5s instead of every frame
        local now = tick()
        if now - _lastCharCheck > 0.5 or not _cachedChar or not _cachedHRP then
            _lastCharCheck = now
            _cachedChar = lp.Character
            _cachedHRP = _cachedChar and _cachedChar:FindFirstChild("HumanoidRootPart")
        end

        local r = _cachedHRP
        if not r or not r.Parent then
            _cachedChar, _cachedHRP = nil, nil
            return
        end
