...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        if ph~=lastPh or lastMv==Vector3.zero then lastMv=mv lastPh=ph hum:Move(mv,false) end
...
do
    ...
    PBC.Position=UDim2.new(0.5,-160,0,52)
    StealBarRef=PBC
    ...
    -- RadiusInput invisible, kept for logic
...
do
    ...
    sbName.Size=UDim2.new(0,90,1,0) sbName.Position=UDim2.new(0,14,0,0)
    ...
    -- separator
    local sbSep=Instance.new("TextLabel",SB)
    sbSep.Size=UDim2.new(0,10,1,0) sbSep.Position=UDim2.new(0,108,0,0)
    sbSep.BackgroundTransparency=1 sbSep.Text="|"
    sbSep.Font=Enum.Font.GothamBold sbSep.TextSize=14 sbSep.TextColor3=ACCENT
    sbSep.TextXAlignment=Enum.TextXAlignment.Center sbSep.ZIndex=11

    -- FPS + PING
    local sbStats=Instance.new("TextLabel",SB)
    sbStats.Size=UDim2.new(1,-125,1,0) sbStats.Position=UDim2.new(0,122,0,0)
    sbStats.BackgroundTransparency=1 sbStats.Text="FPS --  PING --ms"
    sbStats.Font=Enum.Font.GothamBold sbStats.TextSize=13 sbStats.TextColor3=ACCENT
    sbStats.TextXAlignment=Enum.TextXAlignment.Left sbStats.ZIndex=11
    ...
    task.spawn(function()
        ...
        RunService.RenderStepped:Connect(function()
            ...
            if now-lastT>=0.5 then
                ...
                if sbStats and sbStats.Parent then sbStats.Text="FPS "..fps.."  PING "..ping.."ms" end
            end
        end)
    end)
