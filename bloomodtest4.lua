...
local function doTPDown()
    task.spawn(function()
        ...
        if hit then
            ...
            local target=CFrame.new(hit.Position.X,hit.Position.Y+hh+hy+0.1,hit.Position.Z)
...
makeToggle(FeatFrame,"Lock",2,function(v)
    ...
    if AP_LockVisual then AP_LockVisual(v) end
    if v and mobBtnRefs["Lock"] then mobBtnRefs["Lock"].Visible=true end
end)
...
-- Other settings
local function formatRadius(v)
    if v==math.floor(v) then return tostring(math.floor(v)) else return string.format("%.1f",v) end
end
makeSection(SetFrame,"  STEAL",150)
local _,_srb=makeNumInput(SetFrame,"Steal Radius",STEAL_RADIUS,151,function(v) STEAL_RADIUS=math.clamp(v,5,200) if RadiusInput then RadiusInput.Text=formatRadius(STEAL_RADIUS) end end) StealRadiusBox=_srb
...
do
    ...
    PBC.Size=UDim2.new(0,290,0,28)
    PBC.Position=UDim2.new(0.5,-145,0,38)
    StealBarRef=PBC
    ...
    -- Progress bar track — leaves room for radius box and badge on right
    local pt=Instance.new("Frame",PBC)
    pt.Size=UDim2.new(1,-110,1,-10) pt.Position=UDim2.new(0,6,0,5)
    ...
    -- radius input box (editable, left of badge)
    local rBg=Instance.new("Frame",PBC)
    rBg.Size=UDim2.new(0,58,1,-8) rBg.Position=UDim2.new(1,-112,0,4)
    rBg.BackgroundColor3=Color3.fromRGB(180,40,110) rBg.BorderSizePixel=0 rBg.ZIndex=4
    Instance.new("UICorner",rBg).CornerRadius=UDim.new(0,10)
    RadiusInput=Instance.new("TextBox",rBg)
    RadiusInput.Size=UDim2.new(1,0,1,0) RadiusInput.BackgroundTransparency=1
    RadiusInput.Text=formatRadius(STEAL_RADIUS) RadiusInput.TextColor3=WHITE
    RadiusInput.Font=Enum.Font.GothamBold RadiusInput.TextSize=11
    RadiusInput.TextXAlignment=Enum.TextXAlignment.Center RadiusInput.BorderSizePixel=0
    RadiusInput.ClearTextOnFocus=false RadiusInput.ZIndex=5
    RadiusInput.PlaceholderText="R"
    RadiusInput.FocusLost:Connect(function()
        local n=tonumber(RadiusInput.Text)
        if n then
            STEAL_RADIUS=math.clamp(n,5,200)
            RadiusInput.Text=formatRadius(STEAL_RADIUS)
            if StealRadiusBox then StealRadiusBox.Text=formatRadius(STEAL_RADIUS) end
            task.defer(saveConfig)
        else RadiusInput.Text=formatRadius(STEAL_RADIUS) end
    end)
...
do
    ...
    SB.Size=UDim2.new(0,290,0,30)
    SB.Position=UDim2.new(0.5,-145,0,8)
...
-- MOBILE BUTTONS
dropMobBtnRef=nil tpMobBtnRef=nil
autoMedusaMobRef=nil
do
