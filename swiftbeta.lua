...
-- WP ESP: instances created once, only Position/Color updated on refresh
local wpEspParts={} -- keyed by wp name e.g. "L1"
...
local function buildWPEspInstances()
    ...
    wpEspParts={}
    local names={"L1","L2","L3","L4","L5","R1","R2","R3","R4","R5"}
    for _,name in ipairs(names) do
        local p=Instance.new("Part")
        p.Name="WPEsp_"..name p.Anchored=true p.CanCollide=false p.CastShadow=false
        p.Shape=Enum.PartType.Block p.Size=Vector3.new(1,1.8,1) p.Material=Enum.Material.Neon
        p.Parent=wpEspFolder
...
local function refreshWPEsp()
    ...
    -- Build instances on first use or after a disable/destroy
    ...
    for name,data in pairs(wpData) do
        local e=wpEspParts[name]
        if e then
            e.part.Position=data[1]
            e.part.Color=data[2]
            e.lbl.TextStrokeColor3=data[2]
        end
    end
end
...
local function startBatAimbot()
    ...
    -- Turn off autoplay if either side is running
    ...
    -- Velocity history for acceleration-aware prediction
    ...
    aimbotConnection=RunService.Heartbeat:Connect(function(dt)
        ...
        if targetHRP and targetChar then
            ...
            -- Estimate target acceleration from velocity delta
            local now=tick()
            local elapsed=math.max(now-prevTick,0.001)
            local accel=(targetVel-prevVelocity)/elapsed
            prevVelocity=targetVel prevTick=now
            -- Scale predict time by distance: farther away = need to lead more
            local dist=(targetHRP.Position-currentHRP.Position).Magnitude
            local predictTime=math.clamp(dist/math.max(AIMBOT_SPEED,1),0.03,0.35)
            -- Predicted position using velocity + half acceleration (kinematic formula)
            local predictedPos=targetHRP.Position
                +(targetVel*predictTime)
                +(accel*(0.5*predictTime*predictTime))
            -- Full 3D direction to predicted pos
            local dirToTarget=predictedPos-currentHRP.Position
            local dist3D=dirToTarget.Magnitude
            -- Stand position: back off by MELEE_OFFSET along full 3D direction
            local standPos=predictedPos
            if dist3D>0 then standPos=predictedPos-(dirToTarget.Unit*MELEE_OFFSET) end
            -- Face toward predicted position (yaw only)
            ...
            if moveDist>0.5 then
                ...
                -- Y: track full 3D predicted stand position gap, clamped
                local yDelta=standPos.Y-currentHRP.Position.Y
                local yVel=math.clamp(yDelta*12,-80,80)
                -- XZ: decelerate within 3 studs to avoid overshoot
                local xzDist=Vector3.new(moveDir.X,0,moveDir.Z).Magnitude
                local xzSpeed=AIMBOT_SPEED*math.clamp(xzDist/3,0.15,1)
                local flatUnit=Vector3.new(moveUnit.X,0,moveUnit.Z)
            ...
            prevVelocity=Vector3.new(0,0,0) prevTick=tick()
...
local AP_PRE_ROUND_SPD=9    -- speed before round starts
local AP_RoundStarted=false -- true once the round is live

-- Round detection: watches WalkSpeed, ReplicatedStorage values, and GUI text
-- for any signal that the intermission/waiting phase has ended.
local function AP_DetectRoundStart()
    AP_RoundStarted=false
    local function onRoundStart()
        if AP_RoundStarted then return end
        AP_RoundStarted=true
    end
    local function onRoundEnd()
        AP_RoundStarted=false
    end

    -- Method 1: WalkSpeed gate — game sets WalkSpeed >12 when round starts, locks it <=1 during intermission
    local wsConn
    local function watchWalkSpeed()
        ...
        -- If WalkSpeed is already high, round is live
    ...
    -- Method 2: Scan ReplicatedStorage and workspace for Status/GameStatus/RoundStatus values
    local function watchValue(val)
        ...
        check()
        val.Changed:Connect(check)
    end
    local function scanForStatusValues(parent)
        for _,v in ipairs(parent:GetChildren()) do
            local n=v.Name:lower()
            if (n=="status" or n=="gamestatus" or n=="roundstatus" or n=="gamestate" or n=="state" or n=="phase") then
    ...
    -- Method 3: Watch for a countdown/intermission GUI disappearing from PlayerGui
    task.spawn(function()
...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        local tgt=wps[ph]
        local spd=(AP_RoundStarted and WP_L_SPD[ph] or AP_PRE_ROUND_SPD)
        ...
        if dist<AP_REACH then
            ...
            AP_LeftPhase=ph+1; AP_LeftArrived=0; AP_LeftDelayUntil=0
            return
        end
        AP_LeftArrived=0; AP_LeftDelayUntil=0
        -- Recalculate direction every frame so knockback/drift is always corrected
        local d=flat-pos
        local mv=d.Unit
        hum:Move(mv,false)
...
local function AP_StartRight()
    ...
    AP_RightConn=RunService.Heartbeat:Connect(function()
        ...
        local tgt=wps[ph]
        local spd=(AP_RoundStarted and WP_R_SPD[ph] or AP_PRE_ROUND_SPD)
        local pos=rp.Position
        local flat=Vector3.new(tgt.X,pos.Y,tgt.Z)
        local dist=(flat-pos).Magnitude
        if dist<AP_REACH then
            ...
            AP_RightPhase=ph+1; AP_RightArrived=0; AP_RightDelayUntil=0
            return
        end
...
-- Returns the cached base plot name, re-scanning at most every BASE_SCAN_INTERVAL seconds
...
-- Scan a single plot into animalCache; clears old entries for that plot first to avoid duplicates
local function scanPlot(plot)
    if not plot or not plot:IsA("Model") then return end
    local plotName=plot.Name
    -- Remove stale entries for this plot
    for i=#animalCache,1,-1 do if animalCache[i].plot==plotName then table.remove(animalCache,i) end end
    -- Skip our own base
    ...
    for _,pod in ipairs(podiums:GetChildren()) do
        if pod:IsA("Model") and pod:FindFirstChild("Base") then
            ...
            table.insert(animalCache,{plot=plotName,slot=pod.Name,pod=pod,worldPosition=pod:GetPivot().Position,uid=uid})
        end
    end
end

-- Find the steal prompt; uses cached pod reference for speed, falls back to workspace walk
local function findPromptForAnimal(ad)
    if not ad then return nil end
    local cp=promptCache[ad.uid] if cp and cp.Parent then return cp end
    -- Use the cached pod reference if still valid
    local pod=ad.pod
    if not pod or not pod.Parent then
        -- Pod reference is stale; walk workspace to refresh
        local plots=workspace:FindFirstChild("Plots") if not plots then return nil end
        ...
        ad.pod=pod -- update cached reference
    end
...
-- Collect ALL signal connections from a prompt into a flat table keyed by signal name
local function getPromptConnections(prompt)
    local out={hold={},holdEnded={},trigger={},triggered={}}
    pcall(function()
        if not getconnections then return end
        for _,c in ipairs(getconnections(prompt.PromptButtonHoldBegan)) do if type(c.Function)=="function" then table.insert(out.hold,c.Function) end end
        for _,c in ipairs(getconnections(prompt.PromptButtonHoldEnded)) do if type(c.Function)=="function" then table.insert(out.holdEnded,c.Function) end end
        for _,c in ipairs(getconnections(prompt.Triggered)) do if type(c.Function)=="function" then table.insert(out.triggered,c.Function) end end
        -- some games wire TriggerEnded instead of Triggered
        pcall(function()
            for _,c in ipairs(getconnections(prompt.TriggerEnded)) do if type(c.Function)=="function" then table.insert(out.trigger,c.Function) end end
        end)
    end)
    return out
end

-- Fire every hold-start signal variant the server could be listening to
...
-- Fire every hold-end / trigger signal variant
...
-- Check if a podium still has an animal (prompt still exists and enabled)
local function podHasAnimal(prompt)
    return prompt and prompt.Parent and prompt.Enabled
end

-- Execute a steal with retry until the pod is empty or prompt disappears
local function execSteal(prompt)
    ...
    task.spawn(function()
        ...
        while attempt<MAX_RETRIES do
            if not podHasAnimal(prompt) then break end -- already stolen
            attempt=attempt+1

            -- Fetch fresh connections every attempt so they're never stale
            local conns=getPromptConnections(prompt)

            -- Fire hold-begin signals
            fireHoldBegin(prompt,conns)

            -- Hold for the required duration, firing hold signals every tick
            local elapsed=0
            local dur=STEAL_DURATION
            while elapsed<dur do
                ...
                elapsed=elapsed+dt
                -- Re-fire hold callbacks every frame to keep server hold state alive
                for _,f in ipairs(conns.hold) do task.spawn(pcall,f) end
                pcall(function() if prompt.Parent then prompt:InputHoldBegin() end end)
            end

            if not podHasAnimal(prompt) then break end -- stolen mid-hold

            -- Fire all end/trigger variants simultaneously
            fireHoldEnd(prompt,conns)

            -- Brief wait to let server process, then check if it worked
            task.wait(0.08)
            if not podHasAnimal(prompt) then break end

            -- Still there — wait a short cooldown before retrying
            task.wait(0.1)
        end
        ...
        -- Invalidate prompt cache so next scan gets a fresh reference
        if prompt then
...
-- Returns the nearest animal that isn't on our base, with live position update
local function nearestAnimal()
    ...
    for _,ad in ipairs(animalCache) do
        if ad.plot~=myBase then
            -- Update live position from pod if reference is valid
...
local function startAutoSteal()
    ...
    autoStealConn=RunService.Heartbeat:Connect(function()
        ...
        if not podHasAnimal(prompt) then
            -- Prompt gone/disabled, invalidate and rescan next frame
            promptCache[target.uid]=nil lastStealTarget=nil lastStealPrompt=nil
            return
        end
        execSteal(prompt)
    end)
...
task.spawn(function()
    ...
    -- Re-scan new plots as they appear
    plots.ChildAdded:Connect(function(plot)
        if plot:IsA("Model") then task.wait(0.5) scanPlot(plot) end
    end)
    -- Periodic rescan every 5s (scanPlot now deduplicates per-plot)
    task.spawn(function()
...
-- separator
local TitleSep=Instance.new("TextLabel",TitleBar)
TitleSep.Size=UDim2.new(0,12,1,-2) TitleSep.Position=UDim2.new(0,120,0,0)
TitleSep.BackgroundTransparency=1 TitleSep.Text="|"
TitleSep.Font=Enum.Font.Gotham TitleSep.TextSize=13 TitleSep.TextColor3=ACCENT
TitleSep.TextXAlignment=Enum.TextXAlignment.Center TitleSep.TextYAlignment=Enum.TextYAlignment.Center
-- right: fps + ping
local TitleStats=Instance.new("TextLabel",TitleBar)
TitleStats.Size=UDim2.new(1,-140,1,-2) TitleStats.Position=UDim2.new(0,134,0,0)
TitleStats.BackgroundTransparency=1 TitleStats.Text="FPS --  PING --ms"
TitleStats.Font=Enum.Font.GothamBold TitleStats.TextSize=11 TitleStats.TextColor3=PINK_TEXT
TitleStats.TextXAlignment=Enum.TextXAlignment.Left TitleStats.TextYAlignment=Enum.TextYAlignment.Center
...
task.spawn(function()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if now-lastT>=0.5 then
            local fps=math.round(frames/(now-lastT))
            frames=0 lastT=now
            local ping=0
            pcall(function()
                ping=math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
            end)
            if TitleStats and TitleStats.Parent then
                TitleStats.Text="FPS "..fps.."  PING "..ping.."ms"
            end
        end
    end)
end)
...
local function makeTab(parent,txt,order)
    ...
    btn.Size=UDim2.new(0.25,-3,1,0) btn.BackgroundColor3=Color3.fromRGB(240,210,230)
...
local FeatFrame=makeScrollFrame() FeatFrame.Visible=true -- COMBAT (first, visible by default)
local VisFrame=makeScrollFrame()                         -- VISUAL (middle)
local MovFrame=makeScrollFrame()                         -- MOVEMENT (last)
local SetFrame=makeScrollFrame()                         -- SETTINGS
...
makeNumInput(MovFrame,"Pre-Round Speed",AP_PRE_ROUND_SPD,19.5,function(v) AP_PRE_ROUND_SPD=math.max(1,v) end)
...
do
    ...
    PBC.Size=UDim2.new(0,220,0,38)
    PBC.Position=SavedPBCPos and UDim2.new(SavedPBCPos[1],SavedPBCPos[2],SavedPBCPos[3],SavedPBCPos[4]) or UDim2.new(0.5,-110,0,8)
    StealBarRef=PBC
    -- Dark background like second photo
    PBC.BackgroundColor3=Color3.fromRGB(30,30,40) PBC.BackgroundTransparency=0.15 PBC.BorderSizePixel=0
    Instance.new("UICorner",PBC).CornerRadius=UDim.new(0,10)
    local pbs=Instance.new("UIStroke",PBC) pbs.Color=Color3.fromRGB(60,60,80) pbs.Thickness=1 pbs.Transparency=0.4

    -- Hub name
    local hubLbl=Instance.new("TextLabel",PBC)
    hubLbl.Size=UDim2.new(1,-20,0,20) hubLbl.Position=UDim2.new(0,10,0,4)
    hubLbl.BackgroundTransparency=1 hubLbl.Text="Bloom Hub" hubLbl.TextColor3=ACCENT
    hubLbl.Font=Enum.Font.GothamBlack hubLbl.TextSize=11 hubLbl.TextXAlignment=Enum.TextXAlignment.Left hubLbl.ZIndex=3
    -- status (hidden row, kept for logic compatibility)
    ...
    ProgressLabel.TextColor3=Color3.fromRGB(200,200,220) ProgressLabel.Font=Enum.Font.GothamBold ProgressLabel.TextSize=9
    ProgressLabel.Visible=false ProgressLabel.ZIndex=3
    -- steal % (hidden, kept for logic)
    ProgressPctLabel=Instance.new("TextLabel",PBC)
    ProgressPctLabel.Size=UDim2.new(0,0,0,0) ProgressPctLabel.BackgroundTransparency=1 ProgressPctLabel.Text=""
    ProgressPctLabel.Visible=false ProgressPctLabel.ZIndex=3
    -- Radius input (no label, just the value tucked in)
    RadiusInput=Instance.new("TextBox",PBC)
    RadiusInput.Size=UDim2.new(0,28,0,12) RadiusInput.Position=UDim2.new(1,-32,1,-14)
    RadiusInput.BackgroundTransparency=1 RadiusInput.Text=tostring(STEAL_RADIUS)
    RadiusInput.TextColor3=Color3.fromRGB(140,140,160) RadiusInput.Font=Enum.Font.GothamBold RadiusInput.TextSize=9
    ...
    -- Progress bar track: dark bg, pink fill, thin — at very bottom like second photo
    local pt=Instance.new("Frame",PBC)
    pt.Size=UDim2.new(1,-20,0,4) pt.Position=UDim2.new(0,10,1,-6)
    pt.BackgroundColor3=Color3.fromRGB(55,55,70) pt.BorderSizePixel=0 pt.ZIndex=2
    ...
    PBC.InputBegan:Connect(function(input)
        ...
        -- Use input.Changed so drag tracks even when children capture the input
