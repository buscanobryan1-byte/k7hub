...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        if (flat-pos).Magnitude<AP_REACH then
            hum:Move(Vector3.zero,false); rp.AssemblyLinearVelocity=Vector3.zero
            rp.CFrame=rp.CFrame-rp.CFrame.Position+tgt
...
-- Cache callbacks; only rebuild if missing or prompt/connections went stale
local function buildCallbacks(prompt)
    local existing=stealCache[prompt]
    if existing and existing.builtAt and (tick()-existing.builtAt)<5 then return existing end
    local data={hold={},trigger={},ended={},builtAt=tick()}
    pcall(function()
        if getconnections then
            ...
            for _,c in ipairs(getconnections(prompt.PromptButtonHoldEnded)) do if type(c.Function)=="function" then table.insert(data.ended,c.Function) end end
        end
    end)
...
local stealCooldowns={}     -- per-uid: tick() until which target is locked
local activeSteals=0        -- count of in-flight steals (for progress bar)

local function execSteal(prompt,uid)
    if stealCooldowns[uid] and tick()<stealCooldowns[uid] then return false end
    local data=buildCallbacks(prompt) if not data then return false end
    stealCooldowns[uid]=tick()+STEAL_DURATION+0.1
    activeSteals=activeSteals+1
    ...
    progressConn=RunService.Heartbeat:Connect(function()
        if activeSteals<=0 then progressConn:Disconnect() return end
    ...
    task.spawn(function()
        ...
        for _,f in ipairs(data.ended) do task.spawn(pcall,f) end
        activeSteals=math.max(0,activeSteals-1)
        if activeSteals==0 then
            isStealing=false
...
-- Returns the list of in-range targets, sorted nearest-first (squared dist)
local function targetsInRange()
    local h=getHRP() if not h then return nil end
    local hp=h.Position
    local r2=STEAL_RADIUS*STEAL_RADIUS
    local list={}
    for _,ad in ipairs(animalCache) do
        if ad.worldPosition and not isMyBase(ad.plot) then
            local dx=hp.X-ad.worldPosition.X
            local dy=hp.Y-ad.worldPosition.Y
            local dz=hp.Z-ad.worldPosition.Z
            local d2=dx*dx+dy*dy+dz*dz
            if d2<=r2 then list[#list+1]={ad=ad,d2=d2} end
        end
    end
...
local AUTO_STEAL_SCAN_INTERVAL=0.05  -- throttle: ~20 scans/sec
local _lastAutoStealScan=0
local function startAutoSteal()
    if autoStealConn then return end
    autoStealConn=RunService.Heartbeat:Connect(function()
        if not autoStealEnabled then return end
        local now=tick()
        if now-_lastAutoStealScan<AUTO_STEAL_SCAN_INTERVAL then return end
        _lastAutoStealScan=now
        local list=targetsInRange() if not list or #list==0 then return end
        for _,entry in ipairs(list) do
            local ad=entry.ad
            if not stealCooldowns[ad.uid] or now>=stealCooldowns[ad.uid] then
                local prompt=findPromptForAnimal(ad)
                if not prompt then
                    prompt=promptCache[ad.uid]
                    if not prompt or not prompt.Parent then prompt=nil end
                end
                if prompt then
                    promptCache[ad.uid]=prompt
                    execSteal(prompt,ad.uid)
                end
            end
        end
    end)
...
local function stopAutoSteal()
    ...
    isStealing=false activeSteals=0 stealCooldowns={}
    if progressConn then progressConn:Disconnect() progressConn=nil end
    if ProgressBarFill then ProgressBarFill.Size=UDim2.new(0,0,1,0) end
end
