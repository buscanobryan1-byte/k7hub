...
apWaiting=false apWaitConn=nil apPendingRoute=nil
...
batRevengeEnabled=false lastApRoute="left"
...
local function enableRemoveAccessories()
    ...
    if not accessoryConn then
        accessoryConn=player.CharacterAdded:Connect(function(char)
            task.wait(0.5)
    ...
    if accDescConn then accDescConn:Disconnect() end
    accDescConn=workspace.DescendantAdded:Connect(function(obj)
        if removeAccessoriesActive and (obj:IsA("Accessory") or obj:IsA("Hat")) then
            local char=player.Character
            if char and obj:IsDescendantOf(char) then pcall(function() obj:Destroy() end) end
        end
    end)
end
local function disableRemoveAccessories()
    ...
    if accDescConn then accDescConn:Disconnect(); accDescConn=nil end
end
...
local function doStartBoost(route, onStart)
    onStart()
end

local function waitForRoundStart(route, onStart)
    cancelApWait()
    apWaiting=true
    apPendingRoute=route
    if AutoPlayButtonRef then
        AutoPlayButtonRef.BackgroundColor3=Color3.fromRGB(200,140,20)
        AutoPlayButtonRef.Text="WAIT"
    end
    if AutoPlayTitleRef then
        AutoPlayTitleRef.Text="⏳ Auto Play"
    end
    if isRoundActive() then
        apWaiting=false apPendingRoute=nil
        doStartBoost(route, onStart)
        return
    end
    local waitThread=nil
    waitThread=task.spawn(function()
        local timeout=120
        local elapsed=0
        while apWaiting and elapsed<timeout do
            ...
            if hum then
                local ws=hum.WalkSpeed
                -- pre-round state: game sets WalkSpeed to 10
                if ws==10 then
                    cancelApWait()
                    doStartBoost(route, onStart)
                    return
...
-- PATHFINDING HELPER: navigate to target avoiding obstacles, fallback to direct if path fails
local function apFollowPath(targetPos, speed, stopFn)
    local hrp=getHRP() local hum=getHum()
    if not hrp or not hum then return end
    local prevWalkSpeed=hum.WalkSpeed
    hum.WalkSpeed=speed
    local path=PFS:CreatePath({AgentRadius=2,AgentHeight=5,AgentCanJump=false,WaypointSpacing=6})
    local ok=pcall(function() path:ComputeAsync(hrp.Position,targetPos) end)
    local nodes={}
    if ok and path.Status==Enum.PathStatus.Success then
        nodes=path:GetWaypoints()
    else
        nodes={{Position=targetPos}}
    end
    local REACH=2.5
    local STUCK_THRESHOLD=1.5
    local UPDATE_RATE=0.05
    for i,wp in ipairs(nodes) do
        if stopFn() then hum.WalkSpeed=prevWalkSpeed return end
        local flat=Vector3.new(wp.Position.X,0,wp.Position.Z)
        hrp=getHRP() hum=getHum()
        if not hrp or not hum then return end
        local myFlat=Vector3.new(hrp.Position.X,0,hrp.Position.Z)
        if (myFlat-flat).Magnitude<REACH then continue end
        ...
        while true do
            ...
            if (newPos-lastPos).Magnitude<STUCK_THRESHOLD*UPDATE_RATE then
                stuckTime=stuckTime+UPDATE_RATE
    ...
    if hum then hum.WalkSpeed=prevWalkSpeed end
end
...
local function startAutoPlayLeft()
    ...
    task.spawn(function()
        for i,wp in ipairs(waypoints) do
            ...
            apFollowPath(wp.pos, wp.spd, function() return not aplOn end)
            if not aplOn then break end
        ...
        if aplOn then gHum:Move(Vector3.zero,false) gHrp.AssemblyLinearVelocity=Vector3.zero stopAutoPlayLeft() end
    end)
...
local function startAutoPlayRight()
    ...
    task.spawn(function()
        for i,wp in ipairs(waypoints) do
            ...
            apFollowPath(wp.pos, wp.spd, function() return not aprOn end)
            if not aprOn then break end
            if i<#waypoints and AP_DELAY>0 then
                gHum:Move(Vector3.zero,false) gHrp.AssemblyLinearVelocity=Vector3.zero
                task.wait(AP_DELAY)
        ...
        if aprOn then gHum:Move(Vector3.zero,false) gHrp.AssemblyLinearVelocity=Vector3.zero stopAutoPlayRight() end
    end)
...
local function startAutoPlayAwayFromBase()
    ...
    waitForRoundStart(route, function()
        if route=="right" then
...
local revengeConns={}
local function clearRevengeConns()
    for _,c in ipairs(revengeConns) do pcall(function() c:Disconnect() end) end
    revengeConns={}
end
local function setupBatRevengeForChar(c,hum)
    clearRevengeConns()
    local prevHealth=hum.Health
    table.insert(revengeConns,hum.HealthChanged:Connect(function(newHp)
        if not batRevengeEnabled then prevHealth=newHp return end
        if newHp<prevHealth and not batAimbotEnabled then
            ...
            startBatAimbot() updateToggle("Bat Aimbot",true)
        end
        prevHealth=newHp
    end))
    local function hookBatTool(tool)
        if not tool.Name:lower():find("bat") then return end
        local handle=tool:FindFirstChild("Handle")
        if not handle then return end
        table.insert(revengeConns,handle.Touched:Connect(function(hit)
            if not batRevengeEnabled or not batAimbotEnabled then return end
            local hitChar=hit:FindFirstAncestorOfClass("Model")
            if not hitChar or hitChar==c then return end
            local hitHum=hitChar:FindFirstChildOfClass("Humanoid")
            if hitHum and hitHum.Health>0 then
                stopBatAimbot() updateToggle("Bat Aimbot",false)
                task.wait(0.1)
                if lastApRoute=="right" then startAutoPlayRight() updateToggle("Auto Play Right",true)
                else startAutoPlayLeft() updateToggle("Auto Play Left",true) end
            end
        end))
    end
    for _,item in ipairs(c:GetChildren()) do hookBatTool(item) end
    table.insert(revengeConns,c.ChildAdded:Connect(function(item)
        if item:IsA("Tool") then hookBatTool(item) end
    end))
end
...
local medusaCounterMobRef=nil local autoMedusaMobRef=nil
...
makeToggle(FeatFrame,"Bat Revenge",4,function(v) batRevengeEnabled=v end)
makeToggle(FeatFrame,"Medusa Counter",6,function(v)
    medusaCounterEnabled=v
    if medusaCounterMobRef then medusaCounterMobRef.Visible=v end
...
makeSection(VisFrame,"  PERFORMANCE",5)
makeToggle(VisFrame,"Optimizer",6,function(v)
    if v then enableOptimizer() else disableOptimizer() end
end)
makeToggle(VisFrame,"Remove Accessories",7,function(v)
    if v then enableRemoveAccessories() else disableRemoveAccessories() end
end)
...
do
    ...
    do
        ...
        APFrame.Size=UDim2.new(0,100,0,54)
        ...
        playBtn.Size=UDim2.new(1,-8,0,30) playBtn.Position=UDim2.new(0,4,0,19)
    ...
    local medCtrBtn=makeMob("MEDUSA\nCTR",UDim2.new(1,-120,0.5,-170+gap*3),nil,function() useMedusa() end)
    medCtrBtn.Visible=false medusaCounterMobRef=medCtrBtn medCtrBtn.Size=UDim2.new(0,90,0,50)
    local autoMedBtn=makeMob("AUTO\nMED",UDim2.new(1,-120,0.5,-170+gap*4),nil,function()
