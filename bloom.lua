...
local ACCENT  = Color3.fromRGB(180,30,60)
local WHITE   = Color3.fromRGB(255,255,255)
local BG      = Color3.fromRGB(12,5,8)
local CARD    = Color3.fromRGB(22,10,14)
local OFF_CLR = Color3.fromRGB(50,20,28)
local MOB_ON  = Color3.fromRGB(200,35,65)
local MOB_OFF = Color3.fromRGB(18,6,10)
...
local function enableWPEsp()
    ...
    local wpList={
        {"L-P1",POS_L1,ACCENT},{"L-P2",POS_L2,ACCENT},{"L-P3",POS_L3,ACCENT},{"L-P4",POS_L4,ACCENT},{"L-P5",POS_L5,ACCENT},{"L-P6",POS_L6,ACCENT},
        {"R-P1",POS_R1,R_CLR},{"R-P2",POS_R2,R_CLR},{"R-P3",POS_R3,R_CLR},{"R-P4",POS_R4,R_CLR},{"R-P5",POS_R5,R_CLR},{"R-P6",POS_R6,R_CLR},
    }
...
local function AP_GetLeftWP()
    return {POS_L1,POS_L2,POS_L3,POS_L4,POS_L5,POS_L6}
end
local function AP_GetRightWP()
    return {POS_R1,POS_R2,POS_R3,POS_R4,POS_R5,POS_R6}
end
...
local function AP_StartLeft()
    AP_StopLeft(); AP_LeftOn=true; AP_LeftPhase=1
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        if not AP_LeftOn then return end
        local c=player.Character; if not c then return end
        local rp=c:FindFirstChild("HumanoidRootPart"); local hum=c:FindFirstChildOfClass("Humanoid")
        ...
        local flat=Vector3.new(tgt.X,rp.Position.Y,tgt.Z)
        if (flat-rp.Position).Magnitude<AP_REACH then
            AP_LeftPhase=ph+1; return
        end
...
local function AP_StartRight()
    AP_StopRight(); AP_RightOn=true; AP_RightPhase=1
...
MainFrame.Position=UDim2.new(0.5,-135,0.5,-240) MainFrame.BackgroundColor3=BG
MainFrame.BorderSizePixel=0 MainFrame.Active=true MainFrame.Visible=false MainFrame.Parent=ScreenGui
Instance.new("UICorner",MainFrame).CornerRadius=UDim.new(0,16)

local MainStroke=Instance.new("UIStroke",MainFrame)
MainStroke.Thickness=2.5 MainStroke.Color=ACCENT MainStroke.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
task.spawn(function()
    while MainFrame and MainFrame.Parent do
        for i=0,20 do MainStroke.Thickness=2.5+i*0.04 task.wait(0.04) end
        for i=0,20 do MainStroke.Thickness=3.3-i*0.04 task.wait(0.04) end
    end
end)

-- Title bar (cherry dark)
local TitleBar=Instance.new("Frame",MainFrame)
TitleBar.Size=UDim2.new(1,0,0,52) TitleBar.BackgroundColor3=Color3.fromRGB(28,6,12) TitleBar.BorderSizePixel=0
Instance.new("UICorner",TitleBar).CornerRadius=UDim.new(0,16)
local TitleFix=Instance.new("Frame",TitleBar)
TitleFix.Size=UDim2.new(1,0,0,16) TitleFix.Position=UDim2.new(0,0,1,-16)
TitleFix.BackgroundColor3=Color3.fromRGB(28,6,12) TitleFix.BorderSizePixel=0
...
TitleLbl.TextStrokeColor3=ACCENT TitleLbl.TextStrokeTransparency=0.4

-- Single "FEATURES" tab label (decorative, not clickable)
local TabBar=Instance.new("Frame",MainFrame)
TabBar.Size=UDim2.new(1,-20,0,28) TabBar.Position=UDim2.new(0,10,0,56) TabBar.BackgroundTransparency=1
local FeatTabLbl=Instance.new("TextLabel",TabBar)
FeatTabLbl.Size=UDim2.new(1,0,1,0) FeatTabLbl.BackgroundColor3=ACCENT FeatTabLbl.BorderSizePixel=0
FeatTabLbl.Text="FEATURES" FeatTabLbl.Font=Enum.Font.GothamBlack FeatTabLbl.TextSize=12
FeatTabLbl.TextColor3=WHITE
Instance.new("UICorner",FeatTabLbl).CornerRadius=UDim.new(0,8)

-- Single scrolling frame for everything
local function makeScrollFrame()
    local sf=Instance.new("ScrollingFrame",MainFrame)
    sf.Size=UDim2.new(1,-20,1,-94) sf.Position=UDim2.new(0,10,0,90)
...
-- All content goes into one frame
local FeatFrame=makeScrollFrame()
FeatFrame.Visible=true
-- Alias all old separate frames to the single frame so all content populates it
local MovFrame=FeatFrame
local VisFrame=FeatFrame
local SetFrame=FeatFrame
local BindFrame=FeatFrame
...
local function makeNumInput(parent,labelText,defaultVal,order,onChanged)
    ...
    box.BackgroundColor3=Color3.fromRGB(22,6,11) box.Text=tostring(defaultVal)
...
sfBar.Name="SFBar" sfBar.Size=UDim2.new(1,0,0,36) sfBar.BackgroundColor3=Color3.fromRGB(20,5,10) sfBar.BorderSizePixel=0
...
local function makeFloatRow(labelText,getValue,setValue)
    ...
    box.BackgroundColor3=Color3.fromRGB(22,6,11) box.Text=tostring(getValue())
...
do
    ...
    local function makeWPRow(parent,label,order,getPos,setPos)
        ...
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,10)
        Instance.new("UIStroke",row).Color=Color3.fromRGB(120,20,40)
        local lbl=Instance.new("TextLabel",row)
        ...
        local function makeBox(xOff,val,clr)
            ...
            b.BackgroundColor3=Color3.fromRGB(20,5,10) b.Text=string.format("%.2f",val)
        ...
        for i,data in ipairs({{48,"X"},{106,"Y"},{164,"Z"}}) do
            ...
            al.TextColor3=Color3.fromRGB(220,150,160) al.TextXAlignment=Enum.TextXAlignment.Center
        end
    ...
    makeWPRow(MovFrame,"P6:",27,function() return POS_L6 end,function(v) POS_L6=v end)
    makeSideLabel(MovFrame,"  ▶ RIGHT PATH",28)
    makeWPRow(MovFrame,"P1:",29,function() return POS_R1 end,function(v) POS_R1=v end)
    makeWPRow(MovFrame,"P2:",30,function() return POS_R2 end,function(v) POS_R2=v end)
    makeWPRow(MovFrame,"P3:",31,function() return POS_R3 end,function(v) POS_R3=v end)
    makeWPRow(MovFrame,"P4:",32,function() return POS_R4 end,function(v) POS_R4=v end)
    makeWPRow(MovFrame,"P5:",33,function() return POS_R5 end,function(v) POS_R5=v end)
    makeWPRow(MovFrame,"P6:",34,function() return POS_R6 end,function(v) POS_R6=v end)

    -- WAYPOINT SPEED EDITOR
    makeSection(MovFrame,"  WAYPOINT SPEEDS",34)
    do
        local function makeWPSpeedRow(order, label, getLSpd, setLSpd, getRSpd, setRSpd)
            ...
            local function makeBox(xPos, capText, getV, setV, clr)
                ...
                box.BackgroundColor3=Color3.fromRGB(22,6,11) box.Text=tostring(getV())
        ...
        makeWPSpeedRow(35,"P1:", function() return WP_L_SPD[1] end, function(v) WP_L_SPD[1]=v end, function() return WP_R_SPD[1] end, function(v) WP_R_SPD[1]=v end)
        makeWPSpeedRow(36,"P2:", function() return WP_L_SPD[2] end, function(v) WP_L_SPD[2]=v end, function() return WP_R_SPD[2] end, function(v) WP_R_SPD[2]=v end)
        makeWPSpeedRow(37,"P3:", function() return WP_L_SPD[3] end, function(v) WP_L_SPD[3]=v end, function() return WP_R_SPD[3] end, function(v) WP_R_SPD[3]=v end)
        makeWPSpeedRow(38,"P4:", function() return WP_L_SPD[4] end, function(v) WP_L_SPD[4]=v end, function() return WP_R_SPD[4] end, function(v) WP_R_SPD[4]=v end)
        makeWPSpeedRow(39,"P5:", function() return WP_L_SPD[5] end, function(v) WP_L_SPD[5]=v end, function() return WP_R_SPD[5] end, function(v) WP_R_SPD[5]=v end)
        makeWPSpeedRow(40,"P6:", function() return WP_L_SPD[6] end, function(v) WP_L_SPD[6]=v end, function() return WP_R_SPD[6] end, function(v) WP_R_SPD[6]=v end)
    end
    -- Delay input
    makeNumInput(MovFrame,"Delay (s)",AP_DELAY,41,function(n)
        AP_DELAY=math.max(0,n)
    end)
    makeToggle(MovFrame,"Waypoint ESP",42,function(v)
        wpEspEnabled=v
        if v then enableWPEsp() else disableWPEsp() end
    end)

    -- AP WAYPOINT EDITOR (P1-P6, reuses POS_L/R vars)
    makeSection(MovFrame,"  AUTO PLAY WAYPOINTS",50)
    makeSideLabel(MovFrame,"  ◀ AP LEFT PATH",51)
    makeWPRow(MovFrame,"P1:",52,function() return POS_L1 end,function(v) POS_L1=v end)
    makeWPRow(MovFrame,"P2:",53,function() return POS_L2 end,function(v) POS_L2=v end)
    makeWPRow(MovFrame,"P3:",54,function() return POS_L3 end,function(v) POS_L3=v end)
    makeWPRow(MovFrame,"P4:",55,function() return POS_L4 end,function(v) POS_L4=v end)
    makeWPRow(MovFrame,"P5:",56,function() return POS_L5 end,function(v) POS_L5=v end)
    makeWPRow(MovFrame,"P6:",57,function() return POS_L6 end,function(v) POS_L6=v end)
    makeSideLabel(MovFrame,"  ▶ AP RIGHT PATH",58)
    makeWPRow(MovFrame,"P1:",59,function() return POS_R1 end,function(v) POS_R1=v end)
    makeWPRow(MovFrame,"P2:",60,function() return POS_R2 end,function(v) POS_R2=v end)
    makeWPRow(MovFrame,"P3:",61,function() return POS_R3 end,function(v) POS_R3=v end)
    makeWPRow(MovFrame,"P4:",62,function() return POS_R4 end,function(v) POS_R4=v end)
    makeWPRow(MovFrame,"P5:",63,function() return POS_R5 end,function(v) POS_R5=v end)
    makeWPRow(MovFrame,"P6:",64,function() return POS_R6 end,function(v) POS_R6=v end)

    -- AP WAYPOINT SPEED CUSTOMIZER
    makeSection(MovFrame,"  AUTO PLAY WP SPEEDS",65)
    do
        local function makeAPSpeedRow(order, label, getLSpd, setLSpd, getRSpd, setRSpd)
            local row=Instance.new("Frame",MovFrame)
            row.Size=UDim2.new(1,0,0,44) row.BackgroundColor3=CARD row.BorderSizePixel=0 row.LayoutOrder=order
            Instance.new("UICorner",row).CornerRadius=UDim.new(0,10)
            local lbl=Instance.new("TextLabel",row)
        ...
        makeAPSpeedRow(66,"P1:",function() return WP_L_SPD[1] end,function(v) WP_L_SPD[1]=v end,function() return WP_R_SPD[1] end,function(v) WP_R_SPD[1]=v end)
        makeAPSpeedRow(67,"P2:",function() return WP_L_SPD[2] end,function(v) WP_L_SPD[2]=v end,function() return WP_R_SPD[2] end,function(v) WP_R_SPD[2]=v end)
        makeAPSpeedRow(68,"P3:",function() return WP_L_SPD[3] end,function(v) WP_L_SPD[3]=v end,function() return WP_R_SPD[3] end,function(v) WP_R_SPD[3]=v end)
        makeAPSpeedRow(69,"P4:",function() return WP_L_SPD[4] end,function(v) WP_L_SPD[4]=v end,function() return WP_R_SPD[4] end,function(v) WP_R_SPD[4]=v end)
        makeAPSpeedRow(70,"P5:",function() return WP_L_SPD[5] end,function(v) WP_L_SPD[5]=v end,function() return WP_R_SPD[5] end,function(v) WP_R_SPD[5]=v end)
        makeAPSpeedRow(71,"P6:",function() return WP_L_SPD[6] end,function(v) WP_L_SPD[6]=v end,function() return WP_R_SPD[6] end,function(v) WP_R_SPD[6]=v end)
    end
end
...
makeSection(VisFrame,"  SPEED",102)
makeToggle(VisFrame,"Speed Customizer",103,function(v)
...
-- SETTINGS SECTION
makeSection(SetFrame,"  SETTINGS",149)
...
makeSection(SetFrame,"  CONFIG",159)
...
do
    ...
    saveBtn.TextColor3=WHITE saveBtn.BorderSizePixel=0 saveBtn.LayoutOrder=160
...
do
    ...
    bindHint.Size=UDim2.new(1,0,0,36) bindHint.BackgroundColor3=Color3.fromRGB(28,6,12)
    ...
    bindHint.TextColor3=Color3.fromRGB(255,200,210) bindHint.Font=Enum.Font.Gotham
...
for idx,entry in ipairs(bindList) do
    ...
    keyBtn.BackgroundColor3=Color3.fromRGB(40,8,16)
...
do
    ...
    StealBarRef=PBC
    PBC.BackgroundColor3=Color3.fromRGB(16,4,8) PBC.BackgroundTransparency=0.08 PBC.BorderSizePixel=0
    ...
    task.spawn(function()
        ...
        grad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,ACCENT),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(40,8,16)),ColorSequenceKeypoint.new(1,ACCENT)})
    ...
    sep.BackgroundTransparency=1 sep.Text="|" sep.TextColor3=Color3.fromRGB(120,50,65)
    ...
    rl.Text="R:" rl.TextColor3=Color3.fromRGB(220,180,190) rl.Font=Enum.Font.GothamBold rl.TextSize=9 rl.ZIndex=3
    ...
    pt.BackgroundColor3=Color3.fromRGB(28,8,14) pt.BorderSizePixel=0 pt.ZIndex=2
    Instance.new("UICorner",pt).CornerRadius=UDim.new(1,0)
    ProgressBarFill=Instance.new("Frame",pt)
    ProgressBarFill.Size=UDim2.new(0,0,1,0) ProgressBarFill.BackgroundColor3=Color3.fromRGB(200,30,60)
...
do
    ...
    do
        ...
        AP_SetVisual=function(state)
            apBtn.BackgroundColor3=state and Color3.fromRGB(30,200,80) or MOB_OFF
...
do
    ...
    OBtn.BackgroundColor3=Color3.fromRGB(180,30,60) OBtn.Text="M" OBtn.TextSize=26
    OBtn.Font=Enum.Font.GothamBlack OBtn.TextColor3=Color3.fromRGB(255,255,255) OBtn.BorderSizePixel=0 OBtn.Active=true
    Instance.new("UICorner",OBtn).CornerRadius=UDim.new(0,14)
    local OS=Instance.new("UIStroke",OBtn) OS.Thickness=2 OS.Color=Color3.fromRGB(255,255,255) OS.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    task.spawn(function()
    ...
    task.spawn(function()
        local RED=Color3.fromRGB(180,30,60)
        local BRIGHT=Color3.fromRGB(220,60,90)
        while OBtn and OBtn.Parent do
