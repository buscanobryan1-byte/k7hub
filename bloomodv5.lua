...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        if ph~=lastPh or lastMv==Vector3.zero or mv:Dot(lastMv)<0.999 then lastMv=mv lastPh=ph hum:Move(mv,false) end
...
do
    ...
    PBC.Size=UDim2.new(0,260,0,38)
    -- reset if saved position is the old top-bar default (Y scale==0, Y offset<=12)
    if SavedPBCPos and SavedPBCPos[3]==0 and SavedPBCPos[4]<=12 then SavedPBCPos=nil end
    PBC.Position=SavedPBCPos and UDim2.new(SavedPBCPos[1],SavedPBCPos[2],SavedPBCPos[3],SavedPBCPos[4]) or UDim2.new(0.5,-130,0.5,-19)
    StealBarRef=PBC
    ...
    local pbs=Instance.new("UIStroke",PBC) pbs.Color=ACCENT pbs.Thickness=1 pbs.Transparency=0.5
    ...
    -- separator
    local pbSep=Instance.new("TextLabel",PBC)
    pbSep.Size=UDim2.new(0,8,0,20) pbSep.Position=UDim2.new(0,90,0,4)
    pbSep.BackgroundTransparency=1 pbSep.Text="|" pbSep.TextColor3=ACCENT
    pbSep.Font=Enum.Font.GothamBold pbSep.TextSize=13 pbSep.ZIndex=3
    -- FPS + PING
    local pbStats=Instance.new("TextLabel",PBC)
    pbStats.Size=UDim2.new(1,-105,0,20) pbStats.Position=UDim2.new(0,100,0,4)
    pbStats.BackgroundTransparency=1 pbStats.Text="FPS --  PING --ms" pbStats.TextColor3=PINK_TEXT
    pbStats.Font=Enum.Font.GothamBold pbStats.TextSize=11 pbStats.TextXAlignment=Enum.TextXAlignment.Left pbStats.ZIndex=3
    ...
    task.spawn(function()
        ...
        RunService.RenderStepped:Connect(function()
            ...
            if now-lastT>=0.5 then
                ...
                if pbStats and pbStats.Parent then pbStats.Text="FPS "..fps.."  PING "..ping.."ms" end
            end
        end)
    end)
    ...
    -- RadiusInput kept invisible for logic compatibility
    RadiusInput=Instance.new("TextBox",PBC)
    RadiusInput.Size=UDim2.new(0,0,0,0) RadiusInput.BackgroundTransparency=1 RadiusInput.Text=tostring(STEAL_RADIUS)
    RadiusInput.TextTransparency=1 RadiusInput.ZIndex=1 RadiusInput.BorderSizePixel=0 RadiusInput.Visible=false
