...
-- Build callbacks from a prompt; returns a data table or nil
local function buildCallbacks(prompt)
    ...
    if #data.hold>0 or #data.trigger>0 then stealCache[prompt]=data return data end
    return nil
end

-- Execute a steal on a prompt; returns true if started
local function execSteal(prompt)
    -- Re-fetch fresh callbacks every steal attempt so they're never stale
    local data=buildCallbacks(prompt)
    if not data then data=stealCache[prompt] end
    if not data or not data.ready then return false end
    data.ready=false isStealing=true stealStartTime=tick()
    -- Progress bar via lightweight spawn loop (no persistent Heartbeat connection)
    ...
    task.spawn(function()
        local dur=STEAL_DURATION
        -- Fire hold callbacks and native hold begin in parallel
        ...
        -- Re-fetch trigger callbacks fresh at fire time for maximum reliability
        local freshTrigger={}
        pcall(function()
            if getconnections then
                for _,c in ipairs(getconnections(prompt.Triggered)) do if type(c.Function)=="function" then table.insert(freshTrigger,c.Function) end end
                for _,c in ipairs(getconnections(prompt.PromptButtonHoldEnded)) do if type(c.Function)=="function" then table.insert(freshTrigger,c.Function) end end
            end
        end)
        for _,f in ipairs(freshTrigger) do task.spawn(pcall,f) end
        -- Also fire native methods
        pcall(function() if prompt and prompt.Parent then prompt:InputHoldEnd() end end)
        if ProgressBarFill then ProgressBarFill.Size=UDim2.new(0,0,1,0) end
        data.ready=true isStealing=false lastStealTarget=nil lastStealPrompt=nil
    end)
    return true
end
...
local function startAutoSteal()
    ...
    autoStealConn=RunService.Heartbeat:Connect(function()
        ...
        if not target or dist>STEAL_RADIUS then
            -- Clear cached target if we walked out of range
            if lastStealTarget~=target then lastStealTarget=nil lastStealPrompt=nil end
            return
        end
        -- Only re-fetch prompt when target changes (avoid per-frame workspace walks)
        local prompt
