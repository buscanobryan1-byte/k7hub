...
local function doTPDown()
    task.spawn(function()
        local ok,err=pcall(function()
            ...
            -- Cast from slightly above current position to avoid missing the floor we're standing on
            local castOrigin=hrp.Position+Vector3.new(0,2,0)
            local hit=workspace:Raycast(castOrigin,Vector3.new(0,-502,0),rp)
            if hit then
                local hh=hum.HipHeight or 2
                local hy=hrp.Size.Y/2
                local targetCF=CFrame.new(hit.Position.X,hit.Position.Y+hh+hy+0.1,hit.Position.Z)*CFrame.Angles(0,(hrp.CFrame-hrp.CFrame.Position):ToEulerAnglesYXZ())
                -- Apply the CFrame for 3 consecutive frames so physics engine can't fight it back
                for _=1,3 do
...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        if c~=cachedC then
            ...
            lastMv=Vector3.zero lastPh=-1 lastPos=nil
        end
