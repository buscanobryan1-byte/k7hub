...
WP_L_SPD={60,60,29,29,29,29}
WP_R_SPD={60,60,29,29,29,29}
WP_L_DLY={0,0,0,0,0}
WP_R_DLY={0,0,0,0,0}
POS_L1=Vector3.new(-476,-6,-27.6)
POS_L2=Vector3.new(-484.3,-6,-26.8) POS_L3=Vector3.new(-476,-6,-27.6)
POS_L4=Vector3.new(-476.6,-6,-93.65) POS_L5=Vector3.new(-484,-6,-93.65)
POS_L6=Vector3.new(-484,-6,-93.65)
POS_R1=Vector3.new(-476,-6,-27.6)
POS_R2=Vector3.new(-484.3,-6,-26.8) POS_R3=Vector3.new(-476,-6,-27.6)
POS_R4=Vector3.new(-476.6,-6,-93.65) POS_R5=Vector3.new(-484,-6,-93.65)
POS_R6=Vector3.new(-484,-6,-93.65)
...
AIMBOT_SPEED=60 MELEE_OFFSET=2.5
aimbotConnection=nil lockedTarget=nil
local aimbotTPFreeze=0
...
local function startBatAimbot()
    ...
    aimbotConnection=RunService.Heartbeat:Connect(function()
        ...
        if tick()<aimbotTPFreeze then return end
        if targetHRP and targetChar then
            ...
            local dynamicPredictTime=math.clamp(speed/120,0.03,0.18)
            ...
            if distToStandPos>0.25 then
                local flatMove=Vector3.new(moveDir.X,0,moveDir.Z)
                local ySnap=targetHRP.Position.Y-currentHRP.Position.Y
                currentHum.WalkSpeed=AIMBOT_SPEED
                currentHRP.AssemblyLinearVelocity=Vector3.new(flatMove.Unit.X*AIMBOT_SPEED,math.clamp(ySnap*12,-60,60),flatMove.Unit.Z*AIMBOT_SPEED)
            else
                currentHum.WalkSpeed=AIMBOT_SPEED
...
local function AP_StartLeft()
    ...
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        hum.WalkSpeed=spd
        local curY=rp.AssemblyLinearVelocity.Y
        local yDiff=tgt.Y-pos.Y
        local yVel=(curY>3) and curY or (math.abs(yDiff)>0.3 and math.clamp(yDiff*10,-60,60) or curY)
        rp.AssemblyLinearVelocity=Vector3.new(lastMv.X*spd,yVel,lastMv.Z*spd)
    end)
end
...
local function AP_Toggle()
    if AP_LeftOn or AP_RightOn then
        ...
        if batAimbotEnabled then
            stopBatAimbot()
            if AP_LockVisual then AP_LockVisual(false) end
        end
...
-- OPTIMIZER
local smoothMapEnabled=false local smoothMapConn=nil
local noAnimBrainrotEnabled=false local noAnimConn=nil

local SMOOTH_IGNORE={
    EclipseESP=true,EclipseESP_Name=true,EclipseSpeedBB=true,EclipseAimbotESP=true,
    EclipseWPEsp=true,EclipseXDuels=true,EclipseXOpenClose=true,
}
local SMOOTH_ALLOWED_MATERIALS={
    [Enum.Material.SmoothPlastic]=true,[Enum.Material.Plastic]=true,
    [Enum.Material.Neon]=true,[Enum.Material.Glass]=true,
}

local function smoothPart(part)
    if not part:IsA("BasePart") then return end
    if SMOOTH_IGNORE[part.Name] then return end
    local charFolder={}
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr.Character then charFolder[plr.Character]=true end
    end
    if charFolder[part.Parent] or charFolder[part.Parent and part.Parent.Parent] then return end
    for _,child in ipairs(part:GetChildren()) do
        if child:IsA("Texture") or child:IsA("Decal") or child:IsA("SpecialMesh") then
            pcall(function() child:Destroy() end)
        end
    end
    if not SMOOTH_ALLOWED_MATERIALS[part.Material] then
...
local function applySmooth()
    task.spawn(function()
        for _,obj in ipairs(workspace:GetDescendants()) do
            pcall(function() smoothPart(obj) end)
            task.wait()
        end
    end)
    if smoothMapConn then smoothMapConn:Disconnect() end
    smoothMapConn=workspace.DescendantAdded:Connect(function(obj)
        if not smoothMapEnabled then return end
        task.defer(function() pcall(function() smoothPart(obj) end) end)
    end)
end

local function stopSmooth()
    if smoothMapConn then smoothMapConn:Disconnect() smoothMapConn=nil end
end
...
local function stopAnimsOn(model)
    ...
    if not anim then
        anim=model:FindFirstChildWhichIsA("Animator",true)
    end
    if anim then
    ...
    if hum then
        ...
        for _,track in ipairs(hum:GetPlayingAnimationTracks and hum:GetPlayingAnimationTracks() or {}) do
            pcall(function() track:Stop(0) end)
        end
    end
end

local function applyNoAnimBrainrot()
    task.spawn(function()
        for _,obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Model") and not isPlayerChar(obj) then
                pcall(function() stopAnimsOn(obj) end)
            end
            task.wait()
        end
    end)
    if noAnimConn then noAnimConn:Disconnect() end
    noAnimConn=workspace.DescendantAdded:Connect(function(obj)
        if not noAnimBrainrotEnabled then return end
        if obj:IsA("Model") and not isPlayerChar(obj) then
            task.defer(function() pcall(function() stopAnimsOn(obj) end) end)
        elseif obj:IsA("Animator") or obj:IsA("AnimationController") then
            task.defer(function()
                if not noAnimBrainrotEnabled then return end
                local model=obj.Parent and obj.Parent.Parent
                if model and model:IsA("Model") and not isPlayerChar(model) then
                    pcall(function() stopAnimsOn(model) end)
                end
            end)
        end
    end)
...
local function stopNoAnimBrainrot()
...
local savedPostFX={}
local LOW_GFX_LIGHTING={
    Brightness=2,Ambient=Color3.fromRGB(178,178,178),
    OutdoorAmbient=Color3.fromRGB(178,178,178),
    ShadowSoftness=0,FogEnd=100000,FogStart=100000,
    GlobalShadows=false,
}
local LOW_GFX_REMOVE={"BlurEffect","SunRaysEffect","ColorCorrectionEffect","BloomEffect","DepthOfFieldEffect"}

local function applyLowGfx()
    local li=game:GetService("Lighting")
    savedLightingProps={}
    for k,_ in pairs(LOW_GFX_LIGHTING) do
        pcall(function() savedLightingProps[k]=li[k] end)
    end
    for k,v in pairs(LOW_GFX_LIGHTING) do
        pcall(function() li[k]=v end)
    end
    savedPostFX={}
    for _,child in ipairs(li:GetChildren()) do
        local isRemove=false
        for _,t in ipairs(LOW_GFX_REMOVE) do if child:IsA(t) then isRemove=true break end end
        if isRemove then
            table.insert(savedPostFX,{class=child.ClassName,name=child.Name})
...
local function stopLowGfx()
    local li=game:GetService("Lighting")
    for k,v in pairs(savedLightingProps) do
        pcall(function() li[k]=v end)
    end
    for _,child in ipairs(li:GetChildren()) do
        local isRemove=false
        for _,t in ipairs(LOW_GFX_REMOVE) do if child:IsA(t) then isRemove=true break end end
        if isRemove then pcall(function() child.Enabled=true end) end
    end
    savedLightingProps={}
    savedPostFX={}
end
...
makeToggle(FeatFrame,"Lock",2,function(v)
    if lockMobBtnRef then lockMobBtnRef.Visible=v end
end)
...
makeToggle(FeatFrame,"Autoplay",10,function(v)
    if apMobBtnRef then apMobBtnRef.Visible=v end
end)
...
-- OPTIMIZER SECTION
makeSection(FeatFrame,"  OPTIMIZER",110)
makeToggle(FeatFrame,"Smooth Map",111,function(v)
    smoothMapEnabled=v
    if v then applySmooth() else stopSmooth() end
end)
makeToggle(FeatFrame,"No Anim Brainrot",112,function(v)
    noAnimBrainrotEnabled=v
    if v then applyNoAnimBrainrot() else stopNoAnimBrainrot() end
end)
makeToggle(FeatFrame,"Low Graphics",113,function(v)
    lowGfxEnabled=v
    if v then applyLowGfx() else stopLowGfx() end
end)
...
makeToggle(SetFrame,"Menu Lock",158,function(v)
    for _,btn in ipairs(mobileButtons) do btn.Visible=v end
end)
...
-- =========== STATS / PROGRESS BAR (white themed, middle top, locked) ===========
do
    local PBC=Instance.new("Frame",ScreenGui)
    PBC.Size=UDim2.new(0,260,0,36)
    -- Always anchored to middle-top, not draggable
    PBC.Position=UDim2.new(0.5,-130,0,8)
    PBC.AnchorPoint=Vector2.new(0,0)
    StealBarRef=PBC
    -- White background, solid pink stroke
    PBC.BackgroundColor3=WHITE PBC.BackgroundTransparency=0 PBC.BorderSizePixel=0
    Instance.new("UICorner",PBC).CornerRadius=UDim.new(0,10)
    local pbs=Instance.new("UIStroke",PBC) pbs.Color=ACCENT pbs.Thickness=2 pbs.Transparency=0

    -- Left: Hub name
    local hubLbl=Instance.new("TextLabel",PBC)
    hubLbl.Size=UDim2.new(0,88,1,0) hubLbl.Position=UDim2.new(0,8,0,0)
    hubLbl.BackgroundTransparency=1 hubLbl.Text="Bloom Hub" hubLbl.TextColor3=ACCENT
    hubLbl.Font=Enum.Font.GothamBlack hubLbl.TextSize=12 hubLbl.TextXAlignment=Enum.TextXAlignment.Left hubLbl.TextYAlignment=Enum.TextYAlignment.Center hubLbl.ZIndex=3

    -- Middle: FPS
    local fpsLbl=Instance.new("TextLabel",PBC)
    fpsLbl.Size=UDim2.new(0,60,1,0) fpsLbl.Position=UDim2.new(0.5,-30,0,0)
    fpsLbl.BackgroundTransparency=1 fpsLbl.Text="FPS --"
    fpsLbl.TextColor3=ACCENT fpsLbl.Font=Enum.Font.GothamBold fpsLbl.TextSize=12
    fpsLbl.TextXAlignment=Enum.TextXAlignment.Center fpsLbl.TextYAlignment=Enum.TextYAlignment.Center fpsLbl.ZIndex=3

    -- Right: ms (ping)
    local msLbl=Instance.new("TextLabel",PBC)
    msLbl.Size=UDim2.new(0,60,1,0) msLbl.Position=UDim2.new(1,-68,0,0)
    msLbl.BackgroundTransparency=1 msLbl.Text="-- ms"
    msLbl.TextColor3=ACCENT msLbl.Font=Enum.Font.GothamBold msLbl.TextSize=12
    msLbl.TextXAlignment=Enum.TextXAlignment.Right msLbl.TextYAlignment=Enum.TextYAlignment.Center msLbl.ZIndex=3
    ...
    ProgressLabel.TextColor3=ACCENT ProgressLabel.Font=Enum.Font.GothamBold ProgressLabel.TextSize=9
    ...
    -- Progress bar track: light pink bg, pink fill, thin — at very bottom
    local pt=Instance.new("Frame",PBC)
    pt.Size=UDim2.new(1,-20,0,3) pt.Position=UDim2.new(0,10,1,-5)
    pt.BackgroundColor3=Color3.fromRGB(255,200,230) pt.BorderSizePixel=0 pt.ZIndex=2
    ...
    -- live update fps + ping for PBC
    task.spawn(function()
        local lastT2=tick() local frames2=0
        RunService.RenderStepped:Connect(function()
            frames2=frames2+1
            local now=tick()
            if now-lastT2>=0.5 then
                local fps=math.round(frames2/(now-lastT2))
                frames2=0 lastT2=now
                local ping=0
                pcall(function() ping=math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue()) end)
                if fpsLbl and fpsLbl.Parent then fpsLbl.Text="FPS "..fps end
                if msLbl and msLbl.Parent then msLbl.Text=ping.." ms" end
            end
        end)
    end)
    -- PBC is NOT draggable (always fixed at middle top)
end

-- MOBILE BUTTONS
local dropMobBtnRef=nil local tpMobBtnRef=nil local apMobBtnRef=nil local lockMobBtnRef=nil
do
    ...
    local dropInitVisible=(toggleStates["Drop"] and toggleStates["Drop"].state) or false
    dropMobBtn.Visible=dropInitVisible dropMobBtnRef=dropMobBtn dropMobBtn.Size=UDim2.new(0,90,0,50)
    local autoBatBtn=makeMob("LOCK",UDim2.new(1,-120,0.5,-170+gap),"Lock",function()
        local ns=not batAimbotEnabled
        if ns then
    ...
    local lockInitVisible=(toggleStates["Lock"] and toggleStates["Lock"].state) or false
    autoBatBtn.Visible=lockInitVisible lockMobBtnRef=autoBatBtn
    ...
    do
        ...
        local apInitVisible=(toggleStates["Autoplay"] and toggleStates["Autoplay"].state) or false
        apBtn.Visible=apInitVisible apMobBtnRef=apBtn
    ...
    local tpInitVisible=(toggleStates["TP Down"] and toggleStates["TP Down"].state) or false
    tpMobBtn.Visible=tpInitVisible tpMobBtnRef=tpMobBtn tpMobBtn.Size=UDim2.new(0,90,0,50)
end
