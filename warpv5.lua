...
local ToggleIcon, MainFrame, SpeedCustomizer, XrayBase, NoAnim, PlayerESP, AutoPlay, AutoTp, AutoSteal
...
local function saveConfig()
    ...
    saveData.Other.RagdollEnabled = Enabled_AntiRagdoll or false
    ...
    saveData.Other.BatAimbotEnabled = autoBatToggled or false
    
    if AutoSteal then
        saveData.Other.AutoStealEnabled = AutoSteal.Enabled or false
    end

    if BatUI and BatUI.btn then
        saveData.Positions.BatUIBtn = {BatUI.btn.Position.X.Scale, BatUI.btn.Position.X.Offset, BatUI.btn.Position.Y.Scale, BatUI.btn.Position.Y.Offset}
    end
    if TPDown and TPDown.btn then
        saveData.Positions.TPDownBtn = {TPDown.btn.Position.X.Scale, TPDown.btn.Position.X.Offset, TPDown.btn.Position.Y.Scale, TPDown.btn.Position.Y.Offset}
    end
    if DropUI and DropUI.btn then
        saveData.Positions.DropUIBtn = {DropUI.btn.Position.X.Scale, DropUI.btn.Position.X.Offset, DropUI.btn.Position.Y.Scale, DropUI.btn.Position.Y.Offset}
    end
...
local Enabled_AntiRagdoll = false
local AntiRagdollConnections = {}
local _cachedMotor6Ds = {}
local _cachedMotorChar = nil

local function _rebuildMotorCache(char)
    _cachedMotor6Ds = {}
    _cachedMotorChar = char
    if not char then return end
    for _, obj in ipairs(char:GetDescendants()) do
        if obj:IsA("Motor6D") then
            table.insert(_cachedMotor6Ds, obj)
        end
    end
end

local function startAntiRagdoll()
    if AntiRagdollConnections.heartbeat then return end

    if lp.Character then _rebuildMotorCache(lp.Character) end
    AntiRagdollConnections.charAdded = lp.CharacterAdded:Connect(function(char)
        _rebuildMotorCache(char)
    end)

    AntiRagdollConnections.heartbeat = RunService.Heartbeat:Connect(function()
        if not Enabled_AntiRagdoll then return end
        ...
        if hum then
            ...
            if st==Enum.HumanoidStateType.Physics or st==Enum.HumanoidStateType.Ragdoll or st==Enum.HumanoidStateType.FallingDown then
                ...
                if char ~= _cachedMotorChar then _rebuildMotorCache(char) end
                for _, m in ipairs(_cachedMotor6Ds) do
                    if m and m.Parent and not m.Enabled then m.Enabled = true end
                end
            end
        end
    end)
...
local function stopAntiRagdoll()
    if AntiRagdollConnections.heartbeat then AntiRagdollConnections.heartbeat:Disconnect(); AntiRagdollConnections.heartbeat=nil end
    if AntiRagdollConnections.charAdded  then AntiRagdollConnections.charAdded:Disconnect();  AntiRagdollConnections.charAdded=nil  end
    _cachedMotor6Ds = {}; _cachedMotorChar = nil
end
...
local function getClosestPlayerForBat()
...
RunService.Heartbeat:Connect(function()
    ...
    if autoBatToggled and now - _batT >= 1/30 then
        ...
        local c = lp.Character; if c then
            local hrp = c:FindFirstChild("HumanoidRootPart"); if hrp then
                local target = getClosestPlayerForBat()
...
local BatUI = { gui = nil, btn = nil }

local function createBatUI()
    BatUI.gui = Instance.new("ScreenGui")
    BatUI.gui.Name = "BatAimbotUI"
    BatUI.gui.ResetOnSpawn = false
    BatUI.gui.Enabled = false
    BatUI.gui.Parent = CoreGui

    BatUI.btn = Instance.new("TextButton")
    BatUI.btn.Size = UDim2.new(0, 130, 0, 40)
    BatUI.btn.Position = UDim2.new(0.5, -65, 0.5, -60)
    BatUI.btn.Text = "Bat Aimbot: OFF"
    BatUI.btn.Font = Enum.Font.GothamBold
    BatUI.btn.TextSize = 14
    BatUI.btn.TextColor3 = Color3.new(1, 1, 1)
    BatUI.btn.BackgroundColor3 = Color3.fromRGB(0, 160, 0)
    BatUI.btn.AutoButtonColor = true
    BatUI.btn.Parent = BatUI.gui

    local uiCorner = Instance.new("UICorner")
    uiCorner.CornerRadius = UDim.new(0, 8)
    uiCorner.Parent = BatUI.btn

    local dragging, dragStart, startPos

    BatUI.btn.InputBegan:Connect(function(input)
        if LockUIMove then return end
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStart = input.Position
            startPos = BatUI.btn.Position
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            BatUI.btn.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)

    if Config.Positions and Config.Positions.BatUIBtn then
        BatUI.btn.Position = UDim2.new(unpack(Config.Positions.BatUIBtn))
    end

    BatUI.btn.MouseButton1Click:Connect(function()
        autoBatToggled = not autoBatToggled
        if autoBatToggled then
            BatUI.btn.Text = "Bat Aimbot: ON"
            BatUI.btn.BackgroundColor3 = Color3.fromRGB(0, 200, 0)
            local c = lp.Character
            if c then
                local hrp = c:FindFirstChild("HumanoidRootPart")
                if hrp then hrp.AssemblyLinearVelocity = Vector3.zero end
            end
            if AutoPlay and AutoPlay.Active then
...
local function createTPDownUI()
    ...
    TPDown.btn.Size = UDim2.new(0, 130, 0, 40)
    TPDown.btn.Position = UDim2.new(0.5, -65, 0.5, -10)
    TPDown.btn.Text = "TP Down"
    TPDown.btn.Font = Enum.Font.GothamBold
    TPDown.btn.TextSize = 14
    TPDown.btn.TextColor3 = Color3.new(1, 1, 1)
    TPDown.btn.BackgroundColor3 = Color3.fromRGB(0, 160, 0)
...
local DropUI = { gui = nil, btn = nil }

local function createDropUI()
    DropUI.gui = Instance.new("ScreenGui")
    DropUI.gui.Name = "DropUI"
    DropUI.gui.ResetOnSpawn = false
    DropUI.gui.Enabled = false
    DropUI.gui.Parent = CoreGui

    DropUI.btn = Instance.new("TextButton")
    DropUI.btn.Size = UDim2.new(0, 130, 0, 40)
    DropUI.btn.Position = UDim2.new(0.5, -65, 0.5, 40)
    DropUI.btn.Text = "Drop"
    DropUI.btn.Font = Enum.Font.GothamBold
    DropUI.btn.TextSize = 14
    DropUI.btn.TextColor3 = Color3.new(1, 1, 1)
    DropUI.btn.BackgroundColor3 = Color3.fromRGB(0, 160, 0)
    DropUI.btn.AutoButtonColor = true
    DropUI.btn.Parent = DropUI.gui

    local uiCorner = Instance.new("UICorner")
    uiCorner.CornerRadius = UDim.new(0, 8)
    uiCorner.Parent = DropUI.btn

    local dragging, dragStart, startPos

    DropUI.btn.InputBegan:Connect(function(input)
        if LockUIMove then return end
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStart = input.Position
            startPos = DropUI.btn.Position
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
                end
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            DropUI.btn.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)

    if Config.Positions and Config.Positions.DropUIBtn then
        DropUI.btn.Position = UDim2.new(unpack(Config.Positions.DropUIBtn))
    end

    local dropActive = false
    local dropThread = nil

    DropUI.btn.MouseButton1Click:Connect(function()
        if dragging then return end

        if dropActive then
            -- clicked while ON → cancel early
            dropActive = false
            if dropThread then task.cancel(dropThread); dropThread = nil end
            DropUI.btn.Text = "Drop"
            DropUI.btn.BackgroundColor3 = Color3.fromRGB(0, 160, 0)
            return
        end
        ...
        dropActive = true
        DropUI.btn.Text = "Drop: ON"
        DropUI.btn.BackgroundColor3 = Color3.fromRGB(0, 200, 0)

        runDrop()

        dropThread = task.delay(1.5, function()
            dropActive = false
            dropThread = nil
            if DropUI.btn then
                DropUI.btn.Text = "Drop"
...
-- Editable waypoint data (5 points per side, X and Z only)
local WaypointEdit = {
    Right = { -- Plot 7 (right side, Z ~29)
        {x=-474, y=-7, z=29,  speed=58.5},
        {x=-478, y=-6, z=25,  speed=58.5},
        {x=-488, y=-5, z=23,  speed=58.5},
        {x=-474, y=-7, z=29,  speed=29},
        {x=-475, y=-7, z=118, speed=29},
    },
    Left = {  -- Plot 3 (left side, Z ~90)
        {x=-475, y=-7, z=90,  speed=58.5},
        {x=-479, y=-5, z=95,  speed=58.5},
        {x=-486, y=-5, z=97,  speed=58.5},
        {x=-474, y=-7, z=92,  speed=29},
        {x=-474, y=-7, z=-1,  speed=29},
    },
}

local function rebuildAutoPlayWaypoints()
    if not AutoPlay then return end
    local right = {}
    for i, wp in ipairs(WaypointEdit.Right) do
        local entry = {position = Vector3.new(wp.x, wp.y, wp.z), speed = wp.speed}
        table.insert(right, entry)
        if i < #WaypointEdit.Right then
            table.insert(right, {position = Vector3.new(wp.x, wp.y, wp.z), speed = wp.speed})
        end
    end
    local left = {}
    for i, wp in ipairs(WaypointEdit.Left) do
        local entry = {position = Vector3.new(wp.x, wp.y, wp.z), speed = wp.speed}
        table.insert(left, entry)
        if i < #WaypointEdit.Left then
            table.insert(left, {position = Vector3.new(wp.x, wp.y, wp.z), speed = wp.speed})
        end
    end
    AutoPlay.waypoints[7] = right
    AutoPlay.waypoints[3] = left
end
...
local function createAutoPlayUI()
    ...
    AutoPlay.btn.Position = UDim2.new(0.5, -70, 0.5, -110)
    AutoPlay.btn.Text = "AutoPlay: OFF"
...
AddToggle("Combat", "Bat Aimbot", false, function(v)
    if v then
        if not BatUI.gui then
            createBatUI()
        end
        BatUI.gui.Enabled = true
    else
        if BatUI.gui then
            BatUI.gui.Enabled = false
        end
...
AddToggle("Combat", "TP Down", false, function(v)
    if v then
        if not TPDown.gui then
            createTPDownUI()
        end
...
AddToggle("Combat", "Drop", false, function(v)
    if v then
        if not DropUI.gui then
            createDropUI()
        end
        DropUI.gui.Enabled = true
    else
        if DropUI.gui then
            DropUI.gui.Enabled = false
        end
...
-- ═══════════════════════════════════════════════
-- SETTINGS: Lock UI + Save Config (custom rows)
-- ═══════════════════════════════════════════════

local function createSettingsRow(pageName, labelText)
    local Row = create("Frame", {
        Size = UDim2.new(0.92, 0, 0, 50), BackgroundColor3 = COLORS.RowBG,
        BackgroundTransparency = 0.5, Parent = Pages[pageName]
    })
...
-- Lock UI row
do
    local Row = createSettingsRow("Settings", "Lock UI Positions")

    local LockBg = create("Frame", {
        Size = UDim2.new(0, 42, 0, 20), Position = UDim2.new(0.95, -42, 0.5, -10),
        BackgroundColor3 = Color3.fromRGB(40, 45, 55), Parent = Row
    })
    create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = LockBg})
    local LockCircle = create("Frame", {
        Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(0.1, 0, 0.1, 0),
        BackgroundColor3 = Color3.new(1, 1, 1), Parent = LockBg
    })
    create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = LockCircle})
    local LockStatus = create("TextLabel", {
        Size = UDim2.new(0, 30, 0, 20), Position = UDim2.new(0.85, -35, 0.5, -10),
        BackgroundTransparency = 1, Text = "OFF", TextColor3 = COLORS.Border,
        Font = "GothamBold", TextSize = 12, TextXAlignment = "Right", Parent = Row
    })

    local LockBtn = create("TextButton", {Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, Text = "", Parent = Row})
    LockBtn.MouseButton1Click:Connect(function()
        LockUIMove = not LockUIMove
        TweenService:Create(LockBg, TweenInfo.new(0.2), {BackgroundColor3 = LockUIMove and COLORS.Border or Color3.fromRGB(40, 45, 55)}):Play()
        TweenService:Create(LockCircle, TweenInfo.new(0.2), {Position = LockUIMove and UDim2.new(0.55, 0, 0.1, 0) or UDim2.new(0.1, 0, 0.1, 0)}):Play()
        LockStatus.Text = LockUIMove and "ON" or "OFF"
    end)
end

-- Save Config row
do
    local Row = createSettingsRow("Settings", "Save Config")
    ...
    SaveBtn.MouseButton1Click:Connect(function()
        ...
        task.wait(1.5)
        SaveBtn.Text = "Save"
        SaveBtn.BackgroundColor3 = COLORS.Border
    end)

    -- Reset all draggable UI to screen center
    local ResetBtn = create("TextButton", {
        Size = UDim2.new(0, 80, 0, 30), Position = UDim2.new(0.95, -200, 0.5, -15),
        BackgroundColor3 = Color3.fromRGB(40, 45, 55), Text = "Reset Pos", TextColor3 = Color3.new(1,1,1),
        Font = "GothamBold", TextSize = 11, Parent = Row
    })
    ...
    ResetBtn.MouseButton1Click:Connect(function()
        ...
        ToggleIcon.Position    = UDim2.new(0.05, 0, 0.5, 0)
        if AutoPlay.btn  then AutoPlay.btn.Position  = UDim2.new(0.5, -70,  0.5, -110) end
        if BatUI.btn     then BatUI.btn.Position     = UDim2.new(0.5, -65,  0.5, -60)  end
        if TPDown.btn    then TPDown.btn.Position    = UDim2.new(0.5, -65,  0.5, -10)  end
        if DropUI.btn    then DropUI.btn.Position    = UDim2.new(0.5, -65,  0.5,  40)  end
        if SpeedCustomizer.SpeedUI and SpeedCustomizer.SpeedUI.MainFrame then
            SpeedCustomizer.SpeedUI.MainFrame.Position = UDim2.new(0.5, -140, 0.5, -100)
        end
        ResetBtn.Text = "Reset ✔"
        task.wait(1)
...
-- ═══════════════════════════════════════════════
-- SETTINGS: Waypoint Editor (P1–P5 Left / Right)
-- ═══════════════════════════════════════════════
do
    -- live position display
    local posRow = create("Frame", {
        Size = UDim2.new(0.92, 0, 0, 36), BackgroundColor3 = Color3.fromRGB(18, 22, 32),
        BackgroundTransparency = 0.2, Parent = Pages["Settings"]
    })
    create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = posRow})
    create("UIStroke", {Color = COLORS.Border, Thickness = 1, Transparency = 0.7, Parent = posRow})
    create("TextLabel", {
        Text = "📍", Size = UDim2.new(0, 20, 1, 0), Position = UDim2.new(0, 8, 0, 0),
        BackgroundTransparency = 1, TextColor3 = Color3.new(1,1,1),
        Font = "GothamBold", TextSize = 14, Parent = posRow
    })
    local posLabel = create("TextLabel", {
        Text = "My Pos —  X: ?    Z: ?",
        Size = UDim2.new(1, -36, 1, 0), Position = UDim2.new(0, 30, 0, 0),
        BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(180, 220, 255),
        Font = "GothamBold", TextSize = 11, TextXAlignment = "Left", Parent = posRow
    })

    -- update live position every 0.2 s
    local posTimer = 0
    RunService.Heartbeat:Connect(function(dt)
        posTimer = posTimer + dt
        if posTimer < 0.2 then return end
        posTimer = 0
        ...
        if char then
            ...
            if hrp then
                ...
                posLabel.Text = string.format("My Pos —  X: %d    Z: %d", math.round(p.X), math.round(p.Z))
            end
        end
    end)

    -- header + rows
    local function makeHeader(text, col)
        local h = create("Frame", {
            Size = UDim2.new(0.92, 0, 0, 28),
            BackgroundColor3 = col, BackgroundTransparency = 0.4,
            Parent = Pages["Settings"]
        })
        create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = h})
        create("TextLabel", {
            Text = text, Size = UDim2.new(1, -10, 1, 0), Position = UDim2.new(0, 8, 0, 0),
            BackgroundTransparency = 1, TextColor3 = Color3.new(1,1,1),
            Font = "GothamBold", TextSize = 11, TextXAlignment = "Left", Parent = h
        })
    end

    local function makeWaypointRow(side, index)
        local wp = WaypointEdit[side][index]
        ...
        create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = Row})
        create("UIStroke", {Color = COLORS.Border, Thickness = 1, Transparency = 0.85, Parent = Row})

        create("TextLabel", {
            Text = "P"..index, Size = UDim2.new(0, 22, 1, 0), Position = UDim2.new(0, 8, 0, 0),
            BackgroundTransparency = 1, TextColor3 = COLORS.Border,
            Font = "GothamBold", TextSize = 13, TextXAlignment = "Left", Parent = Row
        })

        local function makeInput(label, value, xPos)
            create("TextLabel", {
                Text = label, Size = UDim2.new(0, 14, 0, 18),
                Position = UDim2.new(0, xPos, 0.5, -9),
                BackgroundTransparency = 1, TextColor3 = Color3.fromRGB(150,150,160),
                Font = "GothamBold", TextSize = 10, TextXAlignment = "Left", Parent = Row
            })
            local box = create("TextBox", {
                Size = UDim2.new(0, 62, 0, 28),
                Position = UDim2.new(0, xPos + 14, 0.5, -14),
        ...
        local xBox = makeInput("X", wp.x, 33)
        local zBox = makeInput("Z", wp.z, 114)

        -- "Use" button: paste current position into this waypoint
        local useBtn = create("TextButton", {
            Size = UDim2.new(0, 40, 0, 26),
            Position = UDim2.new(0, 184, 0.5, -13),
            BackgroundColor3 = Color3.fromRGB(30, 80, 50),
            Text = "Use", TextColor3 = Color3.new(1,1,1),
            Font = "GothamBold", TextSize = 10, Parent = Row
        })
        create("UICorner", {CornerRadius = UDim.new(0, 5), Parent = useBtn})
        ...
        useBtn.MouseButton1Click:Connect(function()
            ...
            local p = hrp.Position
            local nx, nz = math.round(p.X), math.round(p.Z)
            xBox.Text = tostring(nx)
            zBox.Text = tostring(nz)
            WaypointEdit[side][index].x = nx
            WaypointEdit[side][index].z = nz
            rebuildAutoPlayWaypoints()
            useBtn.Text = "✔"
            useBtn.BackgroundColor3 = Color3.fromRGB(20, 120, 50)
            task.wait(1)
            useBtn.Text = "Use"
            useBtn.BackgroundColor3 = Color3.fromRGB(30, 80, 50)
        end)

        local function apply()
            local nx = tonumber(xBox.Text)
            local nz = tonumber(zBox.Text)
            if nx then WaypointEdit[side][index].x = nx
            else xBox.Text = tostring(WaypointEdit[side][index].x) end
            if nz then WaypointEdit[side][index].z = nz
            else zBox.Text = tostring(WaypointEdit[side][index].z) end
            if nx or nz then rebuildAutoPlayWaypoints() end
        end

        xBox.FocusLost:Connect(apply)
        zBox.FocusLost:Connect(apply)
    end

    makeHeader("▶  Right Waypoints", Color3.fromRGB(20, 100, 30))
    for i = 1, 5 do makeWaypointRow("Right", i) end

    makeHeader("◀  Left Waypoints", Color3.fromRGB(20, 50, 120))
    for i = 1, 5 do makeWaypointRow("Left", i) end
end
...
local function applyExtraConfig()
    if Config.Other then
        ...
        autoBatToggled = Config.Other.BatAimbotEnabled or false
        Enabled_AntiRagdoll = Config.Other.RagdollEnabled or false
    ...
    if ToggleStates["Anti-Ragdoll"] and Enabled_AntiRagdoll then
        startAntiRagdoll()
    end
