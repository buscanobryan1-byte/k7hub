...
local function apFollowPath(targetPos, speed, stopFn)
    ...
    local path=PFS:CreatePath({AgentRadius=2,AgentHeight=5,AgentCanJump=false,WaypointSpacing=3})
    ...
    for _,wp in ipairs(nodes) do
        while true do
            if stopFn() then return end
            hrp=getHRP() hum=getHum()
            if not hrp or not hum then return end
            local d=Vector3.new(wp.Position.X-hrp.Position.X,0,wp.Position.Z-hrp.Position.Z)
...
TitleLbl.Size=UDim2.new(1,-70,1,0) TitleLbl.Position=UDim2.new(0,10,0,0) TitleLbl.BackgroundTransparency=1 TitleLbl.Text="Mafia Hub"
TitleLbl.Font=Enum.Font.GothamBlack TitleLbl.TextSize=18 TitleLbl.TextColor3=WHITE
TitleLbl.TextXAlignment=Enum.TextXAlignment.Right
TitleLbl.TextStrokeColor3=ACCENT TitleLbl.TextStrokeTransparency=0.5

do
    local guiScale=1.0 local BASE_W,BASE_H=260,420
