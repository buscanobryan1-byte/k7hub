...
ProgressBarFill=nil ProgressLabel=nil ProgressPctLabel=nil RadiusInput=nil stealBarLocked=false
...
makeToggle(SetFrame,"Lock Steal Bar",16,function(v) stealBarLocked=v end)
...
do
    ...
    PBC.Size=UDim2.new(0,240,0,42) PBC.Position=UDim2.new(0.5,-120,1,-100)
    ...
    ProgressLabel.Size=UDim2.new(0.45,0,0.5,0) ProgressLabel.Position=UDim2.new(0,10,0,3)
    ...
    ProgressPctLabel.Size=UDim2.new(0.25,0,0.5,0) ProgressPctLabel.Position=UDim2.new(0.42,0,0,3)
    ...
    rl.Size=UDim2.new(0,44,0,16) rl.Position=UDim2.new(1,-92,0,4) rl.BackgroundTransparency=1
    rl.Text="Radius:" rl.TextColor3=Color3.fromRGB(180,180,200) rl.Font=Enum.Font.GothamBold rl.TextSize=10 rl.ZIndex=3
    RadiusInput=Instance.new("TextBox",PBC)
    RadiusInput.Size=UDim2.new(0,38,0,16) RadiusInput.Position=UDim2.new(1,-46,0,4)
    RadiusInput.BackgroundTransparency=1 RadiusInput.Text=tostring(STEAL_RADIUS)
    RadiusInput.TextColor3=ACCENT RadiusInput.Font=Enum.Font.GothamBold RadiusInput.TextSize=10
    ...
    pt.Size=UDim2.new(0.92,0,0,9) pt.Position=UDim2.new(0.04,0,1,-16)
    ...
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then pbDragging=false end
    end)
end
...
do
    ...
    local function makeMob(label,initPos,toggleName,onAct)
        ...
        btn.Size=UDim2.new(0,58,0,58) btn.Position=initPos
    ...
    local col1=UDim2.new(1,-120,0.5,-170) local col2=UDim2.new(1,-56,0.5,-170)
    local gap=68
    makeMob("DROP",col1,nil,function() task.spawn(doDrop) end)
    makeMob("AUTO\nLEFT",col2,"Auto Play Left",function()
        local ns=not aplOn
        if ns then
            if batAimbotEnabled then stopBatAimbot() updateToggle("Bat Aimbot",false) end
            if aprOn then stopAutoPlayRight() updateToggle("Auto Play Right",false) end
    ...
    makeMob("AUTO\nRIGHT",UDim2.new(1,-56,0.5,-170+gap),"Auto Play Right",function()
        local ns=not aprOn
        if ns then
            if batAimbotEnabled then stopBatAimbot() updateToggle("Bat Aimbot",false) end
            if aplOn then stopAutoPlayLeft() updateToggle("Auto Play Left",false) end
    ...
    makeMob("SLOW\nDOWN",UDim2.new(1,-120,0.5,-170+gap*2),"Slow Down",function()
        local ns=not(toggleStates["Slow Down"] and toggleStates["Slow Down"].state)
        slowDownEnabled=ns updateToggle("Slow Down",ns)
    end)
    makeMob("TP\nDOWN",UDim2.new(1,-56,0.5,-170+gap*2),nil,function() task.spawn(doTPDown) end)
end
