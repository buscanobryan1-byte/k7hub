...
local function execSteal(prompt)
    ...
    task.spawn(function()
        ...
        while attempt<MAX_RETRIES do
            ...
            -- Hold for the required duration, re-firing hold signals every tick
            -- to satisfy servers that check hold state continuously
            local elapsed=0
            local dur=STEAL_DURATION
            while elapsed<dur do
                if not podHasAnimal(prompt) then break end
                RunService.Heartbeat:Wait()
                elapsed=elapsed+RunService.Heartbeat:Wait() -- second wait returns delta
            end
