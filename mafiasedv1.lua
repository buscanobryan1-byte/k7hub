...
local function doStartBoost(route, onStart)
    task.spawn(function()
        -- pre-round: nonstop toward P1 while game WalkSpeed is still 10
        local target=(route=="left") and POS_L1 or POS_R1
        while true do
            local hum=getHum() local hrp=getHRP()
            if not hum or not hrp then break end
            -- game changes WalkSpeed away from 10 when round actually starts
            if hum.WalkSpeed~=10 then break end
            local d=Vector3.new(target.X-hrp.Position.X,0,target.Z-hrp.Position.Z)
            if d.Magnitude>0 then
                local dir=d.Unit
                hum:Move(dir,false)
                hrp.AssemblyLinearVelocity=Vector3.new(dir.X*10,hrp.AssemblyLinearVelocity.Y,dir.Z*10)
            end
            task.wait()
        end
        -- round started: auto-play overrides at custom speed
        onStart()
    end)
end
