...
gChar=nil gHum=nil gHrp=nil speedBB=nil speedLV=nil speedAtt=nil
...
-- START BOOST: on round start detection, walkspeed 8 + small nudge for positional advantage
    ...
    task.spawn(function()
        pcall(function()
            ...
            if hum and hrp then
                ...
                hum.WalkSpeed=8
                local firstWP=(route=="left") and POS_L1 or POS_R1
                local dir=Vector3.new(firstWP.X-hrp.Position.X,0,firstWP.Z-hrp.Position.Z)
                if dir.Magnitude>0 then
                    dir=dir.Unit
                    setSpeedLV(dir.X*8, dir.Z*8)
                end
                task.wait(0.08)
                clearSpeedLV()
                if getHum() then getHum().WalkSpeed=origWS end
            end
        end)
        onStart()
    end)
...
local function advanceApl(next) gHum:Move(Vector3.zero,false) clearSpeedLV() gHrp.AssemblyLinearVelocity=Vector3.zero aplPhase=0 task.delay(AP_DELAY,function() if aplOn then aplPhase=next end end) end
local function startAutoPlayLeft()
    ...
    aplConn=RunService.Heartbeat:Connect(function()
        ...
        elseif aplPhase==1 then
            ...
            setSpeedLV(md.X*NORMAL_SPEED,md.Z*NORMAL_SPEED)
        ...
        elseif aplPhase==3 then
            ...
            local md=d.Unit gHum:Move(md,false)
            setSpeedLV(md.X*SLOW_SPEED,md.Z*SLOW_SPEED)
        ...
        elseif aplPhase==5 then
            ...
            if d.Magnitude<1 then gHum:Move(Vector3.zero,false) clearSpeedLV() gHrp.AssemblyLinearVelocity=Vector3.zero stopAutoPlayLeft() return end
            local md=d.Unit gHum:Move(md,false)
            setSpeedLV(md.X*SLOW_SPEED,md.Z*SLOW_SPEED)
        end
    end)
...
local function advanceApr(next) gHum:Move(Vector3.zero,false) clearSpeedLV() gHrp.AssemblyLinearVelocity=Vector3.zero aprPhase=0 task.delay(AP_DELAY,function() if aprOn then aprPhase=next end end) end
local function startAutoPlayRight()
    ...
    aprConn=RunService.Heartbeat:Connect(function()
        ...
        elseif aprPhase==5 then
            ...
            if d.Magnitude<1 then gHum:Move(Vector3.zero,false) clearSpeedLV() gHrp.AssemblyLinearVelocity=Vector3.zero stopAutoPlayRight() return end
            local md=d.Unit gHum:Move(md,false)
            setSpeedLV(md.X*SLOW_SPEED,md.Z*SLOW_SPEED)
        end
    end)
...
-- LINEAR VELOCITY CONSTRAINT HELPERS
local function setupSpeedConstraint()
    pcall(function() if speedLV then speedLV:Destroy() end end)
    pcall(function() if speedAtt then speedAtt:Destroy() end end)
    speedLV=nil speedAtt=nil
    if not gHrp then return end
    speedAtt=Instance.new("Attachment")
    speedAtt.Name="EclipseSpeedAtt" speedAtt.Parent=gHrp
    speedLV=Instance.new("LinearVelocity")
    speedLV.Name="EclipseSpeedLV"
    speedLV.Attachment0=speedAtt
    speedLV.RelativeTo=Enum.ActuatorRelativeTo.World
    speedLV.VelocityConstraintMode=Enum.VelocityConstraintMode.Plane
    speedLV.PrimaryTangentAxis=Vector3.new(1,0,0)
    speedLV.SecondaryTangentAxis=Vector3.new(0,0,1)
    speedLV.PlaneVelocity=Vector2.zero
    speedLV.MaxForce=1e6
    speedLV.Parent=gHrp
end
local function setSpeedLV(x,z)
    if speedLV and speedLV.Parent then speedLV.PlaneVelocity=Vector2.new(x,z) end
end
local function clearSpeedLV()
    if speedLV and speedLV.Parent then speedLV.PlaneVelocity=Vector2.zero end
end
...
local function setupChar(c)
    ...
    task.wait(0.5) makeSpeedBB() setupSpeedConstraint()
...
do
    ...
    do
        ...
        APFrame.Size=UDim2.new(0,120,0,60)
