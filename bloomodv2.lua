...
local AP_ARRIVE_CD=0.25

local function AP_GetLeftWP()
    return {POS_L1,POS_L2,POS_L3,POS_L4,POS_L5}
end
local function AP_GetRightWP()
    return {POS_R1,POS_R2,POS_R3,POS_R4,POS_R5}
end
...
local function AP_StartLeft()
    AP_StopLeft(); AP_LeftOn=true; AP_LeftPhase=1; AP_LeftArrived=0
    AP_LeftConn=RunService.Heartbeat:Connect(function()
        ...
        if (flat-rp.Position).Magnitude<AP_REACH then
            local now=tick()
            if AP_LeftArrived==0 then AP_LeftArrived=now end
            if now-AP_LeftArrived>=AP_ARRIVE_CD then
                AP_LeftPhase=ph+1; AP_LeftArrived=0
            else
                hum:Move(Vector3.zero,false); rp.AssemblyLinearVelocity=Vector3.zero
            end
            return
...
local function AP_StartRight()
    AP_StopRight(); AP_RightOn=true; AP_RightPhase=1; AP_RightArrived=0
    AP_RightConn=RunService.Heartbeat:Connect(function()
        ...
        if (flat-rp.Position).Magnitude<AP_REACH then
            local now=tick()
            if AP_RightArrived==0 then AP_RightArrived=now end
            if now-AP_RightArrived>=AP_ARRIVE_CD then
                AP_RightPhase=ph+1; AP_RightArrived=0
            else
                hum:Move(Vector3.zero,false); rp.AssemblyLinearVelocity=Vector3.zero
            end
            return
